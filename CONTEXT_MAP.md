...
| **Importadores APIKey migrados a CronApiKey** | `core/api/public_auth.py`, `core/api/public_views.py`, `core/api/public_serializers.py`, `tests/test_api_keys_webhooks.py` | ~~4 de 5 importadores cambiados a `CronApiKey as APIKey`.~~ **REVERTIDO** — CronApiKey no tiene `rate_limit`, `plan`, `scopes`, `user`, `request_count` que estos módulos usan. Se restauraron imports a `core.models.api_keys.APIKey` por compatibilidad. Pendiente: migración gradual cuando CronApiKey soporte todos los atributos. |

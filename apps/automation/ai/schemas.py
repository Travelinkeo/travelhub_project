...
class TramoVueloSchema(BaseModel):
    ...
    codigo_iata_origen: str | None = Field(
        description="Código IATA de 3 letras de la ciudad de origen (ej. BOG, CCS)"
    )
    ...
    localizador_aerolinea: str | None = Field(
        description="Localizador específico de la aerolínea si difiere del principal"
    )
...
class BoletoAereoSchema(BaseModel):
    ...
    codigo_identificacion: str | None = Field(
        description="FOID, DNI, Cédula o Pasaporte del pasajero (sin prefijos)"
    )
    ...
    fecha_emision: str | None = Field(
        description="Fecha de emisión en formato DDMMMAA (ej: 14MAR26)"
    )
    agente_emisor: str | None = Field(
        description="Código IATA o Identificador de la oficina/agente emisor"
    )
    ...
    direccion_aerolinea: str | None = Field(
        description="Dirección física de la aerolínea (si está presente)"
    )
...
class CedulaOCRSchema(BaseModel):
    ...
    nombres: str | None = Field(
        default=None, description="Solo los nombres del titular, en mayúsculas."
    )
    ...
    portrait_bbox: list[int] | None = Field(
        default=[0, 0, 0, 0],
        description="Coordenadas normalizadas [ymin, xmin, ymax, xmax] del rostro.",
    )
...
class ConsejosIASchema(BaseModel):
    ...
    consejo_estrategico: str = Field(
        description="Un consejo de negocio basado en si las ventas suben o bajan"
    )
    accion_recomendada: str = Field(
        description="Una recomendación directa (ej. 'Inicia trámites de Tax Refund para flujo de caja extra')"
    )

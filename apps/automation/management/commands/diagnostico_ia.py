...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"❌ Error: Boleto with ID {boleto_id} does not exist in the database."))
            return

        # 2. Print initial database state
        self.stdout.write(self.style.WARNING(f"\n📋 [INITIAL DB STATE]"))
        self.stdout.write(f" - Estado de Parseo: {boleto.estado_parseo}")
        self.stdout.write(f" - Archivo asociado: {boleto.archivo_boleto.name if boleto.archivo_boleto else 'None'}")
        ...
        self.stdout.write(f" - ID (Monkey Patch Check): {getattr(boleto, 'id', 'Falta atributo id')}")
        ...
        if boleto.archivo_boleto:
            ...
            try:
                ...
                if hasattr(boleto.archivo_boleto, 'seek'):
                    ...
                    self.stdout.write(self.style.SUCCESS(" ✅ Stream rewind completed successfully. Position reset to Byte 0."))
                else:
            ...
            self.stdout.write(self.style.ERROR(f" ❌ Warning: No file associated with this ticket record."))
        ...
        if success:
            ...
            self.stdout.write(self.style.SUCCESS(f" ⏱️ Total execution duration: {duration:.4f} seconds"))
            self.stdout.write(self.style.WARNING(f"\n📋 [FINAL DB STATE]"))
            self.stdout.write(f" - Estado de Parseo: {boleto.estado_parseo}")
            self.stdout.write(f" - Venta Asociada: {boleto.venta_asociada_id or 'None'}")
            self.stdout.write(f" - Datos Parseados: {bool(boleto.datos_parseados)} (Campos: {list(boleto.datos_parseados.keys()) if boleto.datos_parseados else 'Vacío'})")
            self.stdout.write(f" - Log de Parseo Final: {boleto.log_parseo or 'Sin logs/Vacío.'}")
            if str(boleto.estado_parseo) in ['ERR', 'REV']:
                self.stdout.write(self.style.ERROR(f" ❌ Processing finished with parsing status: {boleto.estado_parseo}"))
            else:
                self.stdout.write(self.style.SUCCESS(f" ✅ Processing finished with status: {boleto.estado_parseo}"))

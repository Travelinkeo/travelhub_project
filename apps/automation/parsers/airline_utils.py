...
def get_airline_name_by_code(codigo_iata: str) -> str | None:
    """
    ...
    try:
        aerolinea = Aerolinea.objects.filter(
            codigo_iata__iexact=codigo_iata.upper(), activa=True
        ).first()
...
def extract_airline_code_from_flight(numero_vuelo: str) -> str | None:
    """
...
def normalize_airline_name(
    raw_name: str, flight_number: str = None, ticket_number: str = None
) -> str:
    """
    ...
    EXCLUDED_WORDS = {
        ...
        "AR",
        "AP",
        "RP",  # Sufijos corporativos y agente emisor
    }
...
def validate_airline_numeric_code(ticket_number: str, airline_iata: str) -> bool:
    ...
    try:
        ...
        if aerolinea.codigo_numerico == prefix:
            ...
            logger.warning(
                f"⚠️ Discrepancia Validación Numérica: Boleto {ticket_number} (Prefijo {prefix}) != {airline_iata} (Código {aerolinea.codigo_numerico})"
            )
            return False

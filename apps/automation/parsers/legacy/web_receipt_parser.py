...
class WebReceiptParser(BaseTicketParser):
    """
    ...
    def can_parse(self, text: str) -> bool:
        ...
        if "AVIOR" in text_upper and (
            "RESERVA" in text_upper
            or "LOCALIZADOR" in text_upper
            or "PNR" in text_upper
            or "CONFIRMACION" in text_upper
            or "CONFIRMATION" in text_upper
            or "BOOKING" in text_upper
        ):
            return True
        ...
        # Estelar
        if "ESTELAR" in text_upper and (
            "TICKETS ESTELAR" in text_upper or "TICKETS EMITIDOS" in text_upper
        ):
            return True
    ...
    def _clean_money(self, val_str: str) -> Decimal:
        ...
        try:
            ...
            logger.debug("Error convirtiendo monto '%s' a Decimal", val_str, exc_info=True)
            return Decimal(0)
    ...
    def parse(self, text: str, html_text: str = "") -> Any:
        ...
        # Fallback if text_content is too short (BeautifulSoup parsing plain text issues)
        ...
        if "KIUSYS" in text_content or "PASSENGER ITINERARY RECEIPT" in text_content:
            logger.debug("WebReceiptParser saltado (Detectado Standard KIU/GDS)")
            return None
        ...
        if "AVIOR" in text_upper and (
            ...
            "RUTACA AIRLINES" in text_content
            or "TICKETS EMITIDOS CON ÉXIT O" in text_content
            or "TICKETS RUTACA" in text_content
        ):
            if not soup.find("table"):
                logger.debug("Branch Rutaca Text detected")
                result = self._parse_rutaca_text(text_content)

        # 2. Detección HTML
        elif "ESTELAR" in text_content and (
            "TICKETS ESTELAR" in text_content or "TICKETS EMITIDOS" in text_content
        ):
            logger.debug("Branch Estelar HTML detected")
        ...
        # Se agrego soporte para bookingId
            ...
            soup.find(id="passengerName")
            and soup.find(id="ticket")
            and (soup.find(id="pnr") or soup.find(id="bookingId"))
        ):
            logger.debug("Branch Avior HTML detected")
            result = self._parse_avior(soup)

        elif "RUTACA" in text_content or "TICKETS RUTACA" in text_content:
            logger.debug("Branch Rutaca HTML detected")
        ...
        if result:
            ...
            if not is_valid:
                logger.debug(
                    "WebReceiptParser extracted PENDIENTE ticket. Returning None to trigger Fallback."
                )
                return None
            return MultiParsedTicketData(result)

        logger.debug("No recognizable format found in WebReceiptParser")
    ...
    def _get_airline_data_from_db(
        self, airline_name: str, default_agent: str, default_address: str
    ) -> dict[str, str]:
        """
    ...
    def _parse_avior_text(self, text: str) -> dict[str, Any]:
        ...
        route_header = get_match(
            r"Informaci.n del viaje:[\s\r\n]*([^\r\n]*(?:[\r\n]+[^\r\n]*)*?)(?:\s+Trayecto|\s+Fecha|\s+Salida|\s+Ordenado)",
            group=1,
        )
        if not route_header:
            # Fallback simple
            route_header = get_match(r"Informaci.n del viaje:\s*(.*)")
        ...
        delimiter_pattern = r"(?:Correo|Email|Documento|Document|Doc|Id|Tiquete|Ticket|Tel|Phone|N.mero|Nmero|Nº|N°)"

        match_block = re.search(
            f"{header_pattern}\\s*(.*?)\\s*(?:{delimiter_pattern}(?:\\s+\\w+)*\\s*:)",
            text,
            ...
            for i, line in enumerate(lines):
                ...
                if ("NOMBRE:" in line_upper or "NAME:" in line_upper) and (
                    ...
                    if i + 1 < len(lines):
                        ...
                        if (
                            i + 2 < len(lines)
                            and "@" not in raw_name_line
                            and len(raw_name_line) < 20
                        ):
                        ...
                        if len(extracted.split()) == 1 and i + 2 < len(lines):
                            ...
                            if (
                                next_line_cand
                                and ":" not in next_line_cand
                                and not any(char.isdigit() for char in next_line_cand)
                            ):
        ...
        nombres = [
            re.sub(r"<[^>]+>", "", n).strip()
            for n in nombres
            if n.strip() and "tiquete" not in n.lower()
        ]
        ...
        total_group = (
            total_match.group(1)
            if total_match
            else get_match(r"Total\s.*?(?:USD|VES)?\s*([\d\s.,]+)", group=1)
        )
        total_val = self._clean_money(re.sub(r"(\d)\s+(\d)", r"\1\2", total_group))

        # Bilingual Taxes
        tax_group = get_match(
            r"(?:Tasas \+ impuestos|Taxes|Taxes \+ Fees|Impuestos)\s*(?:USD|VES)?\s*([\d\s.,]+)",
            group=1,
        )
            ...
            vuelos_data.append(
                {
                    ...
                    "fecha": fecha_display if d_match else "VER RUTA",
            ...
            tickets.append(
                {
                    ...
                    "NUMERO_DE_BOLETO": unique_tickets[i]
                    if i < len(unique_tickets)
                    else "PENDIENTE",
                    "NOMBRE_DEL_PASAJERO": nombres[i].upper()
                    if i < len(nombres)
                    else "PASAJERO DESCONOCIDO",
                    "SOLO_NOMBRE_PASAJERO": (
                        nombres[i].split("/")[1] if "/" in nombres[i] else nombres[i].split()[0]
                    )
                    if i < len(nombres)
                    else "",
    ...
    def _parse_estelar(self, soup: BeautifulSoup) -> dict[str, Any]:
        """
        Parsea boletos de Estelar (Formato Web / Manifiesto).
        Soporta múltiples pasajeros y navegación defensiva de DOM.
        """
        logger.info("Detectado formato Web: ESTELAR (Manifiesto)")

        # 1. Extracción de Totales (Consolidado)
        total_monto = Decimal(0)
        total_node = soup.find(string=re.compile(r"Total", re.I))
        if total_node:
            # Navegación defensiva: buscar valor en siguiente <p> o celda
            parent = total_node.parent
            val_node = None
            if parent:
                # Caso p -> p (sibling)
                val_node = parent.find_next_sibling("p")
                if not val_node:
                    # Caso td -> td (sibling)
            ...
            if val_node:
                total_monto = self._clean_money(val_node.get_text(strip=True))

        # 2. Extracción de PNR (Global)
        pnr = "NO_PNR"
        pnr_labels = soup.find_all(string=re.compile(r"Código de reserva", re.I))
        for pnr_label in pnr_labels:
            # Estructura: <p>Código...</p> <p>VYAHKY</p> o similar
            parent = pnr_label.parent
            val_node = None
            if parent:
                # Comprobamos si el valor está en el siguiente hermano (p, a, span)
            ...
            if val_node:
                cand = val_node.get_text(strip=True).upper()
                # Un PNR suele tener 5-8 caracteres
                if 5 <= len(cand) <= 8 and cand.isalnum():
                    pnr = cand
                    break

        # 3. Extracción de Pasajeros y Tickets
        tickets = []
        # Buscamos todos los labels de ticket
        ticket_labels = soup.find_all(string=re.compile(r"Número de ticket", re.I))
        ...
        for label in ticket_labels:
            ...
            if label_p:
                # Búsqueda de valor del ticket
                # Opción A: Siguiente <p> (mismo contenedor/celda)
                val_node = label_p.find_next_sibling("p")
                if not val_node:
                    # Opción B: Siguiente TD
                    label_td = label_p.find_parent("td")
                    if label_td:
            ...
            # Office ID / Airline data logic
            db_data = self._get_airline_data_from_db(
                "ESTELAR",
                "CCS00ESKA",
                "TORRE ESTELAR CALLE LONDRES EDIF LAS MERCEDES, BARUTA, VENEZUELA",
            )

            tickets.append(
                {
                    "SOURCE_SYSTEM": "ESTELAR_WEB",
                    "NUMERO_DE_BOLETO": tkt_num,
    ...
    def _parse_avior(self, soup: BeautifulSoup) -> dict[str, Any]:
        ...
        if not pax_nodes:
            ...
            if single_pax:
                pax_nodes = [single_pax]
            else:
                return {"error": "No se encontraron pasajeros (passengerName)"}
        ...
        base_val = (
            self._clean_money(soup.find(id="amountBeforeTax").get_text(strip=True))
            if soup.find(id="amountBeforeTax")
            else (total_val - tax_val)
        )
        ...
        for i, pax_node in enumerate(pax_nodes):
            ...
            db_data = self._get_airline_data_from_db(
                "AVIOR", "BLA009VWW", "AV JORGE RODRIGUEZ CC MT NIVEL PB LC 35 ANZOATEGUI"
            )
    ...
    def _parse_rutaca(self, soup: BeautifulSoup) -> dict[str, Any]:
        ...
        logger.debug("_parse_rutaca STARTED")
        ...
        db_data = self._get_airline_data_from_db(
            "RUTACA",
            "BLA005RML",
            "AV JESUS SOTO SECTOR AEROPUERTO EDIF TALLER MARES, CIUDAD BOLIVAR, VE",
        )
        ticket_data["AGENTE_EMISOR"] = db_data["agente"]
        ticket_data["DIRECCION_AEROLINEA"] = db_data["direccion"]

        return {"is_multi_pax": True, "tickets": [ticket_data]}

    def _extract_flights_common(
        self, soup: BeautifulSoup, airline_name: str = None
    ) -> list[dict[str, Any]]:
        """
        ...
        for row in valid_tables:
            ...
            if flight_info_table:
                ...
                if nv_label:
                    ...
                    if val_p:
                        numero_vuelo = val_p.get_text(strip=True)
                ...
                if cl_label:
                    ...
                    if val_cl:
                        clase = val_cl.get_text(strip=True)
    ...
    def _parse_rutaca_text(self, text: str) -> dict[str, Any]:
        ...
        pnr_match = re.search(
            r"ago\s*de\s*r\s*e\s*s\s*e\s*r\s*v\s*a\s*:?\s*([A-Z0-9]{6})", text, re.I
        )
        if not pnr_match:
        ...
        try:
            ...
            found_names = re.findall(
                r"Nombr\s*e\s+(.*?)\s*Apellido\s+(.*?)\s*Corr\s*eo", text, re.DOTALL | re.I
            )
        ...
        # 5. Vuelos (Nuevo motor basado en segmentos iterativo)
        vuelos_data = []
        viaje_match = re.search(
            r"Detalles\s*de\s*viaje:(.*?)(?:Detalles\s*de\s*Pago|Prep[aá]rate\s*para)",
            text,
            re.DOTALL | re.I,
        )
        viaje_text = viaje_match.group(1) if viaje_match else text

        pattern = r"([^\n]+)\n\s*Hora\s*de\s*salida:\s*(\d{2}:\d{2}\s*(?:am|pm|AM|PM))\n([^\n]+).*?([^\n]+)\n\s*Hora\s*de\s*llegada:\s*(\d{2}:\d{2}\s*(?:am|pm|AM|PM))\n([^\n]+).*?Nro\s*de\s*vuelo\n\s*(\d+)\n\s*Clase\n\s*([A-Z0-9]+)"
        ...
        for match in re.finditer(pattern, viaje_text, re.DOTALL | re.IGNORECASE):
            origen = match.group(1).strip()
            hora_salida = match.group(2).replace(" ", "").upper()
            fecha_salida_raw = match.group(3).strip()
            destino = match.group(4).strip()
            hora_llegada = match.group(5).replace(" ", "").upper()
            numero_vuelo = match.group(7).strip()
            clase = match.group(8).strip()

            fecha_iso = mt.datetime.now().strftime("%Y-%m-%d")
            fecha_display = mt.datetime.now().strftime("%d-%m-%Y")

            date_match = re.search(r"(\d{1,2})\s+(?:de|De)\s+([A-Za-z]+)", fecha_salida_raw, re.I)
            if date_match:
                day = date_match.group(1).zfill(2)
                month_name = date_match.group(2).lower()
                for m_es, m_num in meses.items():
                    if m_es in month_name:
                        year = mt.datetime.now().year
                        fecha_iso = f"{year}-{m_num}-{day}"
                        fecha_display = f"{day}-{m_num}-{year}"
                        break
        ...
        if not vuelos_data:
            logger.warning(
                "No se encontraron segmentos con el nuevo patrón de Rutaca Text. Usando fallback."
            )
            vuelos_data.append(
                {
                    "origen": "RUTACA_ORG",
                    "destino": "RUTACA_DES",
                    "numero_vuelo": "N/A",
                    "fecha": mt.datetime.now().strftime("%d-%m-%Y"),
                    "fecha_salida": mt.datetime.now().strftime("%Y-%m-%d"),
                    "hora_salida": "00:00",
                    "hora_llegada": "00:00",
                    "clase": "Y",
                    "aerolinea": "RUTACA AIRLINES",
        ...
        for i in range(effective_pax):
            ...
            # Assign all segments to this pax
            import copy

            vuelos_pax = copy.deepcopy(vuelos_data)
            for v in vuelos_pax:
                v["aerolinea"] = airline_name

...
def _parse_currency_amount(value: str) -> tuple[str | None, Decimal | None]:
    ...
    if m:
        ...
        try:
            ...
            logger.warning(f"Formato de monto inválido en '{value}': {e}")
            amount = None
...
def _inferir_fecha_llegada(fecha_salida: str, hora_salida: str, hora_llegada: str, fecha_llegada_existente: str | None) -> str:
    ...
    if hora_salida and hora_llegada:
        try:
            ...
            logger.warning(f"Error al inferir fecha de llegada para salida {fecha_salida} {hora_salida}: {e}")
    return base.strftime('%d-%m-%Y')

def _parse_date_flexible(date_str: str, reference_date: dt.datetime | None = None, base_year: int = None) -> str:
    """

...
class TarifarioParser:
    ...
    def extraer_hoteles(self):
        ...
        for _i, page in enumerate(self.reader.pages):
            ...
            # Detectar hotel
            if (
                "TARIFA COMISIONABLE" in texto
                or "COMISIONABLE AL" in texto
                or "AGENCIAS COMISIONABLE" in texto
            ):
        ...
        try:
            ...
            for patron in patrones_nombres:
                ...
                if match:
                    ...
                    # Filtrar inválidos
                    if (
                        "WWW" not in nombre_candidato
                        and "TARIFA" not in nombre_candidato
                        and "COMISIONABLE" not in nombre_candidato
                        and "VIGENCIA" not in nombre_candidato
                        and len(nombre_candidato) > 3
                    ):
    ...
    def _extraer_ubicacion(self, texto):
        ...
        match = re.search(
            r"(SOLO DESAYUNO|TODO INCLUIDO|MEDIA PENSIÓN|PENSIÓN COMPLETA|SOLO ALOJAMIENTO)\s+(.*?)\s+VIGENCIA",
            texto,
            re.DOTALL,
        )
        if match:
    ...
    def _extraer_politicas(self, texto):
        ...
        # Política de niños
        match = re.search(
            r"POLÍTICA DE NIÑOS:(.*?)(?:CHECK IN|$)", texto, re.DOTALL | re.IGNORECASE
        )
        if match:
    ...
    def _extraer_tarifas(self, texto):
        ...
        # Detectar temporadas
        lineas = texto.split("\n")
        for _i, linea in enumerate(lineas):
            # Detectar temporada
            ...
            match = re.search(
                r"(\d{2}/\d{2}/\d{4})\s*AL\s*(\d{2}/\d{2}/\d{4})([A-Z\s\(\)]+?)(\$|€)([\d,]+)\s+(\$|€)([\d,]+)\s+(\$|€)([\d,]+)\s+(\$[\d,]+|€[\d,]+|N/A)\s+(\$|€)([\d,]+)",
                linea,
            )
            if match:
                try:

...
class VentaBuilderService:
    ...
    def construir_venta_multipax(agencia, boletos_data: list, cliente_pagador: Cliente) -> Venta:
        ...
        pnr = (
            primer_boleto.get("codigo_reserva") or primer_boleto.get("CODIGO_RESERVA") or "SIN-PNR"
        )
        moneda_codigo = str(
            primer_boleto.get("moneda") or primer_boleto.get("TOTAL_MONEDA") or "USD"
        )[:3].upper()

        moneda, _ = Moneda.objects.get_or_create(
            codigo_iso=moneda_codigo, defaults={"nombre": moneda_codigo}
        )
    ...
    def _crear_item_venta(venta: Venta, boleto: BoletoImportado, datos: dict):
        ...
        num_boleto = (
            datos.get("numero_boleto")
            or datos.get("NUMERO_DE_BOLETO")
            or boleto.numero_boleto
            or "N/A"
        )
        nombre_pax = (
            datos.get("nombre_pasajero")
            or datos.get("NOMBRE_DEL_PASAJERO")
        ...
        ItemVenta.objects.update_or_create(
            ...
            defaults={
                ...
                "impuestos_item_venta": Decimal(
                    str(datos.get("impuestos") or datos.get("IMPUESTOS") or 0)
                ),
            },
        )

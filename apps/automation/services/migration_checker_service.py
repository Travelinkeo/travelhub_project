...
class MigrationCheckerService:
    ...
    def check_migration_requirements(
        self, pasajero_id: int, vuelos: list[dict[str, Any]], venta_id: int | None = None, user=None
    ) -> "Any":
        """
        ...
        Pasajero = apps.get_model("crm", "Pasajero")
        try:
        ...
        passport_validity_ok, passport_months = self._check_passport_validity(
            pasajero.fecha_vencimiento_documento, fecha_viaje
        )
        ...
        if not passport_validity_ok:
            ...
            validation_result.summary = (
                f"⚠️ PASAPORTE INSUFICIENTE ({passport_months} meses). " + validation_result.summary
            )
        ...
        MigrationCheck = apps.get_model("core", "MigrationCheck")
        ...
        logger.info(
            f"{migration_check.get_alert_emoji()} Validación completada: {pasajero.get_nombre_completo()} → {destino}"
        )
    ...
    def _get_nationality_code(self, pasajero: "Any") -> str:
    ...
    def _check_passport_validity(
        self, passport_expiry: date | None, travel_date: date | None, min_months: int = 6
    ) -> tuple[bool, int]:
        """
    ...
    def _build_result_from_local_rule(
        self, rule: dict[str, Any], passport_ok: bool
    ) -> MigrationValidationResult:

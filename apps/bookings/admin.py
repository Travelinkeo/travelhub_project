...
class ItemVentaInline(admin.TabularInline):
    ...
    def vincular_componente(self, obj):
            ...
            if hasattr(obj, rel_name):
                try:
                    ...
                    for related in related_queryset:
                        url = reverse(
                            f"admin:bookings_{related._meta.model_name}_change", args=[related.pk]
                        )
                        links.append(
                            f'<a href="{url}" style="font-weight: bold; color: #4f46e5;">[{label}]</a>'
                        )
                except Exception:
                    continue
        ...
        if not links:
            return format_html(
                '<span style="color: #999; font-style: italic;">Sin componente detallado</span>'
            )
...
class AlquilerAutoReservaInline(admin.StackedInline):
    ...
    fieldsets = (
        (
            ...
            {
                "fields": (
                    ("compania_rentadora", "categoria_auto"),
                    ("numero_confirmacion", "nombre_conductor"),
                )
            },
        ),
        (
            ...
            {
                "fields": (
                    ("fecha_hora_retiro", "ciudad_retiro"),
                    ("fecha_hora_devolucion", "ciudad_devolucion"),
                )
            },
        ),
        (
...
class ServicioAdicionalDetalleInline(admin.StackedInline):
    ...
    fieldsets = (
        ...
        ("Costos y Precios", {"fields": (("costo_neto", "precio_venta"),)}),
        (
    ...
    def save_model(self, request, obj, form, change):
        ...
        if change:
            try:
                ...
                generate_ticket = import_string(
                    "apps.automation.parsers.ticket_parser.generate_ticket"
                )
...
class AlojamientoReservaAdmin(SaaSAdminMixin, admin.ModelAdmin):
    ...
    def generar_voucher_hotel(self, request, queryset):
        ...
        if pdf_bytes:
            ...
            messages.error(
                request, f"No se pudo generar el voucher para {reserva.nombre_establecimiento}."
            )
...
class AlquilerAutoReservaAdmin(SaaSAdminMixin, admin.ModelAdmin):
    ...
    def generar_voucher_auto(self, request, queryset):
        ...
        if pdf_bytes:
            ...
            messages.error(
                request, f"No se pudo generar el voucher para el alquiler {alquiler.pk}."
            )
...
class ServicioAdicionalDetalleAdmin(SaaSAdminMixin, admin.ModelAdmin):
    ...
    def generar_voucher_servicio_action(self, request, queryset):
        ...
        if pdf_bytes:
            ...
            messages.error(
                request, f"No se pudo generar el voucher para el servicio {servicio.pk}."
            )
...
class TrasladoServicioAdmin(SaaSAdminMixin, admin.ModelAdmin):
    ...
    def generar_voucher_traslado_action(self, request, queryset):
        ...
        if pdf_bytes:
            ...
            messages.error(
                request, f"No se pudo generar el voucher para el traslado {traslado.pk}."
            )
...
class ActividadServicioAdmin(SaaSAdminMixin, admin.ModelAdmin):
    ...
    def generar_voucher_actividad_action(self, request, queryset):
        ...
        if pdf_bytes:
            ...
            messages.error(
                request, f"No se pudo generar el voucher para la actividad {actividad.pk}."
            )
...
# --- Catalog Admins required for autocomplete_fields ---

from .models import ProductoServicio, Proveedor  # noqa: E402

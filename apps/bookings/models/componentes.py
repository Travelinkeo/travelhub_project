...
class AlojamientoReserva(AgenciaMixin, SoftDeleteModel, models.Model):
    ...
    regimen_alimentacion = models.CharField(
        _("Régimen Alimentación"),
        max_length=30,
        blank=True,
        null=True,
        help_text=_("Ej: Desayuno, Media Pensión, Todo Incluido"),
    )
    habitaciones = models.PositiveSmallIntegerField(_("Habitaciones"), default=1)
    ciudad = models.ForeignKey(
        Ciudad, on_delete=models.PROTECT, verbose_name=_("Ciudad"), null=True, blank=True
    )
    proveedor = models.ForeignKey(
        Proveedor, on_delete=models.SET_NULL, blank=True, null=True, verbose_name=_("Proveedor")
    )
...
class TrasladoServicio(AgenciaMixin, SoftDeleteModel, models.Model):
    ...
    tipo_traslado = models.CharField(
        _("Tipo Traslado"), max_length=3, choices=TipoTraslado.choices, default=TipoTraslado.ARRIBO
    )
...
class ActividadServicio(AgenciaMixin, SoftDeleteModel, models.Model):
    ...
    duracion_horas = models.DecimalField(
        _("Duración (horas)"), max_digits=5, decimal_places=2, blank=True, null=True
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(
                fields=["is_deleted", "agencia_id"], name="idx_actividad_soft_delete_saas"
            ),
        ]
...
class SegmentoVuelo(AgenciaMixin, SoftDeleteModel, models.Model):
    ...
    cabina = models.CharField(
        _("Cabina"),
        max_length=20,
        blank=True,
        null=True,
        help_text=_("Ej: Economy, Business, First"),
    )
    notas = models.TextField(_("Notas"), blank=True, null=True)
...
class AlquilerAutoReserva(AgenciaMixin, SoftDeleteModel, models.Model):
    ...
    ciudad_retiro = models.ForeignKey(
        Ciudad,
        related_name="bookings_autos_retiro",
        on_delete=models.SET_NULL,
        blank=True,
        null=True,
        verbose_name=_("Ciudad Retiro"),
    )
    ciudad_devolucion = models.ForeignKey(
        Ciudad,
        related_name="bookings_autos_devolucion",
        on_delete=models.SET_NULL,
        blank=True,
        null=True,
        verbose_name=_("Ciudad Devolución"),
    )
    ...
    compania_rentadora = models.CharField(
        _("Compañía Rentadora"), max_length=100, blank=True, null=True
    )
    numero_confirmacion = models.CharField(
        _("Número Confirmación"), max_length=100, blank=True, null=True
    )
    ...
    costo_neto = models.DecimalField(
        _("Costo Neto"), max_digits=12, decimal_places=2, blank=True, null=True
    )
    precio_venta = models.DecimalField(
        _("Precio Venta"), max_digits=12, decimal_places=2, blank=True, null=True
    )
    ...
    def clean(self):
        if (
            ...
            and self.fecha_hora_devolucion < self.fecha_hora_retiro
        ):
            raise ValidationError(
                {
                    "fecha_hora_devolucion": _(
                        "La fecha/hora de devolución no puede ser anterior al retiro."
                    )
                }
            )
...
class EventoServicio(AgenciaMixin, SoftDeleteModel, models.Model):
    ...
    codigo_boleto_evento = models.CharField(
        _("Código Boleto / Ref"), max_length=100, blank=True, null=True
    )
    notas = models.TextField(_("Notas"), blank=True, null=True)
    costo_neto = models.DecimalField(
        _("Costo Neto"), max_digits=12, decimal_places=2, blank=True, null=True
...
class CircuitoTuristico(AgenciaMixin, models.Model):
    ...
    incluye = models.TextField(_("Incluye"), blank=True, null=True)
    no_incluye = models.TextField(_("No Incluye"), blank=True, null=True)
    costo_neto_estimado = models.DecimalField(
        _("Costo Neto Estimado"), max_digits=12, decimal_places=2, blank=True, null=True
    )
    precio_venta_estimado = models.DecimalField(
        _("Precio Venta Estimado"), max_digits=12, decimal_places=2, blank=True, null=True
    )
...
class CircuitoDia(AgenciaMixin, models.Model):
    ...
    ciudad = models.ForeignKey(
        Ciudad, on_delete=models.SET_NULL, blank=True, null=True, verbose_name=_("Ciudad")
    )
    alojamiento_previsto = models.CharField(
        _("Alojamiento Previsto"), max_length=255, blank=True, null=True
    )
...
class PaqueteAereo(AgenciaMixin, models.Model):
    ...
    resumen_componentes = models.JSONField(
        _("Resumen Componentes"),
        blank=True,
        null=True,
        help_text=_("Estructura agregada de vuelos/hoteles/otros."),
    )
...
class CruceroReserva(AgenciaMixin, models.Model):
    ...
    tipo_cabina = models.CharField(
        _("Tipo de Cabina"), max_length=3, choices=TipoCabina.choices, default=TipoCabina.BALCON
    )
    ...
    paquete_restaurantes = models.BooleanField(
        _("Paquete Restaurantes Especialidad"), default=False
    )
    ...
    proveedor = models.ForeignKey(
        ...
        blank=True,
        null=True,
        verbose_name=_("Proveedor/Consolidador"),
    )
    localizador_proveedor = models.CharField(_("Localizador Proveedor"), max_length=100, blank=True)
    tarifa_base_cabina = models.DecimalField(
        _("Tarifa Base Cabina"), max_digits=12, decimal_places=2, blank=True, null=True
    )
    ...
    monto_comision = models.DecimalField(
        _("Monto Comisión"), max_digits=12, decimal_places=2, blank=True, null=True, editable=False
    )
    fee_servicio_agencia = models.DecimalField(
        _("Fee Servicio Agencia"), max_digits=12, decimal_places=2, blank=True, null=True
    )
    costo_total_proveedor = models.DecimalField(
        _("Costo Total Proveedor"), max_digits=12, decimal_places=2, blank=True, null=True
    )
    precio_venta_cliente = models.DecimalField(
        _("Precio Venta Cliente"), max_digits=12, decimal_places=2, blank=True, null=True
    )
    moneda = models.ForeignKey(
        "common.Moneda", on_delete=models.PROTECT, verbose_name=_("Moneda"), null=True, blank=True
    )
...
class ServicioAdicionalDetalle(AgenciaMixin, models.Model):
    ...
    id_servicio_adicional = models.AutoField(
        primary_key=True, verbose_name=_("ID Servicio Adicional")
    )
    venta = models.ForeignKey(
        "bookings.Venta",
    ...
    tipo_servicio = models.CharField(
        _("Tipo Servicio"),
        max_length=3,
        choices=TipoServicioChoices.choices,
        default=TipoServicioChoices.OTRO,
    )
    descripcion = models.CharField(_("Descripción"), max_length=255, blank=True, null=True)
    codigo_referencia = models.CharField(
        _("Código Referencia"), max_length=100, blank=True, null=True
    )
    fecha_inicio = models.DateField(_("Fecha Inicio"), blank=True, null=True)
    fecha_fin = models.DateField(_("Fecha Fin"), blank=True, null=True)

# apps/bookings/models/tarifario.py
"""
...
class TarifarioProveedor(AgenciaMixin, models.Model):
    ...
    archivo_pdf = models.FileField(
        upload_to="tarifarios/%Y/%m/", blank=True, null=True, verbose_name=_("Archivo PDF")
    )
    ...
    comision_estandar = models.DecimalField(
        max_digits=5,
        decimal_places=2,
        default=Decimal("15.00"),
        verbose_name=_("Comisión Estándar %"),
    )
...
class ComisionOverrideAerolinea(models.Model):
    ...
    tarifario = models.ForeignKey(
        TarifarioProveedor, on_delete=models.CASCADE, related_name="overrides_aerolinea"
    )
    aerolinea = models.ForeignKey(
        "common.Aerolinea", on_delete=models.CASCADE, verbose_name=_("Aerolínea")
    )
    comision_porcentaje = models.DecimalField(
        max_digits=5, decimal_places=2, verbose_name=_("Comisión Especial %")
    )
...
class HotelTarifario(AgenciaMixin, models.Model):
    """Hotel extraido de un tarifario (o cargado manualmente)"""

    tarifario = models.ForeignKey(
        TarifarioProveedor, on_delete=models.CASCADE, related_name="hoteles", null=True, blank=True
    )
    nombre = models.CharField(max_length=200, verbose_name=_("Nombre del Hotel"))
    slug = models.SlugField(max_length=250, null=True, blank=True, help_text="URL amigable")
    ...
    coordenadas_mapa = models.CharField(
        max_length=100, blank=True, help_text="Lat,Long para Google Maps"
    )

    # Contenido Rico (IA Vision)
    descripcion_corta = models.TextField(
        blank=True, verbose_name=_("Descripción Corta"), help_text="Para listados"
    )
    descripcion_larga = models.TextField(blank=True, verbose_name=_("Descripción Completa (IA)"))

    # Media
    imagen_principal = models.ImageField(
        upload_to="hoteles/portadas/", blank=True, null=True, verbose_name=_("Imagen Portada")
    )
    logo = models.ImageField(
        upload_to="hoteles/logos/", blank=True, null=True, verbose_name=_("Logo del Hotel")
    )
    ...
    categoria = models.IntegerField(
        choices=CATEGORIA_CHOICES, default=3, verbose_name=_("Categoría")
    )
    ...
    regimen_default = models.CharField(
        max_length=2, choices=REGIMEN_CHOICES, default="SD", verbose_name=_("Régimen Principal")
    )
    comision = models.DecimalField(
        max_digits=5, decimal_places=2, verbose_name=_("Comisión %"), default=Decimal("10.00")
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.slug:
            ...
            slug = base_slug
            counter = 1
            # Check for duplicates within the same agency
            while (
                HotelTarifario.all_objects.filter(agencia=self.agencia, slug=slug)
                .exclude(pk=self.pk)
                .exists()
            ):
...
class ImagenHotel(models.Model):
    ...
    hotel = models.ForeignKey(
        HotelTarifario, on_delete=models.CASCADE, related_name="imagenes", null=True, blank=True
    )
...
class TipoHabitacion(models.Model):
    ...
    nombre = models.CharField(
        max_length=100, verbose_name=_("Tipo de Habitación")
    )  # Ej: Matrimonial Standard
...
class TarifaHabitacion(models.Model):
    ...
    tipo_habitacion = models.ForeignKey(
        TipoHabitacion, on_delete=models.CASCADE, related_name="tarifas", null=True, blank=True
    )
    ...
    moneda = models.CharField(
        max_length=3, choices=MONEDA_CHOICES, default="USD", verbose_name=_("Moneda")
    )
    ...
    tarifa_sgl = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, verbose_name=_("SGL (Simple)")
    )
    tarifa_dbl = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, verbose_name=_("DBL (Doble)")
    )
    tarifa_tpl = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, verbose_name=_("TPL (Triple)")
    )
    tarifa_cpl = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, verbose_name=_("CPL (Cuadruple)")
    )

    tarifa_nino = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, verbose_name=_("Tarifa Niño")
    )

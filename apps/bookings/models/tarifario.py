...
class TarifarioProveedor(AgenciaMixin, models.Model):
    ...
    proveedor = models.ForeignKey('bookings.Proveedor', on_delete=models.CASCADE, related_name='tarifarios', null=True, blank=True)
...
class ComisionOverrideAerolinea(models.Model):
    ...
    aerolinea = models.ForeignKey('common.Aerolinea', on_delete=models.CASCADE, verbose_name=_("Aerolínea"))
...
class HotelTarifario(AgenciaMixin, models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            while HotelTarifario.all_objects.filter(agencia=self.agencia, slug=slug).exclude(pk=self.pk).exists():

...
class BoletoImportadoService:
    ...
    def trigger_parsing_if_needed(boleto_importado):
        ...
        if getattr(boleto_importado, "_skip_auto_parse", False):
            logger.info(
                f"⏭️ BoletoImportadoService: Bypass activado para Boleto {boleto_importado.pk}."
            )
            return False
        ...
        if boleto_importado.archivo_boleto and not boleto_importado.datos_parseados:
            try:
                ...
                if updated_count > 0:
                    ...
                    logger.info(
                        f"🧩 BoletoImportadoService: Lock adquirido para Boleto {boleto_importado.pk}. Disparando Celery..."
                    )
                    from django.db import transaction

                    transaction.on_commit(
                        lambda: parsear_boleto_individual.delay(boleto_importado.pk)
                    )
                    return True
                else:
                    logger.info(
                        f"🧩 BoletoImportadoService: Boleto {boleto_importado.pk} ya no está PENDIENTE o Lock falló."
                    )
    ...
    def post_parse_automation(boleto_importado):
        ...
        with transaction.atomic():
            try:
                ...
                # --- Auto-Invoicing ---
                if (
                    boleto_importado.formato_detectado
                    ...
                    try:
                        ...
                        ticket_invoicing_requested.send(
                            ...
                            formato_detectado=boleto_importado.formato_detectado,
                            agencia_id=venta.agencia_id,
                        )
                    except Exception as e_fact:
                        logger.error(
                            f"⚠️ BoletoImportadoService: Error emitiendo evento facturación automática: {e_fact}"
                        )

                # --- Notifications ---
                if boleto_importado.archivo_pdf_generado and not boleto_importado.telegram_file_id:
                    try:
                        from apps.common.tasks import notificar_boleto_procesado_task

                        notificar_boleto_procesado_task.delay(boleto_importado.pk)
                    except Exception as e_notif:
                        logger.error(
                            f"⚠️ BoletoImportadoService: Error encolando notificación: {e_notif}"
                        )

                return venta
            except Exception as e:
                logger.error(
                    f"❌ BoletoImportadoService: Error crítico en post_parse_automation para Boleto {boleto_importado.pk}: {e}"
                )
                return None
    ...
    def evaluate_tax_refund(boleto_importado):
        ...
        if boleto_importado.estado_parseo == "COM":
            try:
                ...
                evaluar_tax_refund_task = import_string(
                    "apps.finance.tasks_tax_refund.evaluar_tax_refund_task"
                )

                transaction.on_commit(lambda: evaluar_tax_refund_task.delay(boleto_importado.pk))
                logger.info(
                    f"💰 BoletoImportadoService: Tax Refund task queued for Boleto {boleto_importado.pk}"
                )
                return True

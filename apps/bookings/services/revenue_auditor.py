"""Servicio de revenue auditor para la aplicación bookings.
"""
...
class RevenueAuditorService:
    ...
    def audit_venta(self, venta: Venta, persist=True):
        ...
        items_sin_costo = venta.items_venta.filter(
            Q(costo_neto_proveedor__isnull=True) | Q(costo_neto_proveedor=0)
        )
    ...
    def run_full_audit(self, days=30, agencia=None):
        ...
        if agencia:
            ventas = ventas.filter(agencia=agencia)
        report = {
        ...
        for venta in ventas:
            ...
            if findings:
                ...
                report["detalles"].append(
                    {"venta_id": venta.pk, "localizador": venta.localizador, "findings": findings}
                )

        logger.info(
            f"AUDITORIA COMPLETADA: {report['total_ventas_auditadas']} auditadas, {report['ventas_con_fugas']} fugas."
        )
        return report

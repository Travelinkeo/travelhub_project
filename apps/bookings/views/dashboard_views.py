...
from core.api import DashboardRateThrottle, cache_api_response, get_user_active_agency
...
@extend_schema(
    ...
    responses={
        200: {
            "description": "Métricas del dashboard con resumen, tendencia semanal, ventas por estado/tipo/canal"
        }
    },
    tags=["Dashboard"],
)
...
def dashboard_metricas(request):
    ...
    ventas_por_estado = list(
        ...
        .annotate(count=Count("id_venta"), total=Sum("total_venta"))
        .order_by("-count")
    )
    ...
    top_clientes = list(
        ...
        .annotate(total_compras=Sum("total_venta"), num_ventas=Count("id_venta"))
        .order_by("-total_compras")[:5]
    )

    ventas_por_tipo = list(
        ventas_qs.values("tipo_venta").annotate(count=Count("id_venta"), total=Sum("total_venta"))
    )

    ventas_por_canal = list(
    ...
    ventas_7_dias = (
        ventas_qs.filter(fecha_venta__date__gte=hace_7_dias, fecha_venta__date__lte=hoy)
        .annotate(fecha_trunc=TruncDate("fecha_venta"))
        .values("fecha_trunc")
        .annotate(total=Sum("total_venta"), count=Count("id_venta"))
        .order_by("fecha_trunc")
    )

    # Build complete 7-day trend (including days with 0 sales)
    tendencia_dict = {
        str(item["fecha_trunc"]): {"total": float(item["total"] or 0), "count": item["count"]}
        for item in ventas_7_dias
    }
    ...
    for i in range(7):
        ...
        dia_str = str(dia)
        if dia_str in tendencia_dict:
            tendencia_semanal.append(
                {
                    "fecha": dia.isoformat(),
                    "total": tendencia_dict[dia_str]["total"],
                    "count": tendencia_dict[dia_str]["count"],
                }
            )
        else:
            tendencia_semanal.append({"fecha": dia.isoformat(), "total": 0, "count": 0})
...
def dashboard_alertas(request):
    ...
    boletos_sin_venta = BoletoImportado.objects.filter(
        agencia=agencia, estado_parseo="COM", venta_asociada__isnull=True
    ).count()
...
class DashboardView(LoginRequiredMixin, View):
    def get_vendedor_dashboard(self, request, agencia):
        ...
        meta_monto = 20000
        porcentaje_meta = (
            min(int((monto_total_mes / meta_monto) * 100), 100) if meta_monto > 0 else 0
        )

        # Leads Calientes (Exclusivos del vendedor y su agencia)
        leads_calientes = (
            OportunidadViaje.objects.filter(agencia=agencia, etapa__in=["NEW", "QUO", "PAY"])
        ...
        ventas_recientes = (
            Venta.objects.filter(agencia=agencia, creado_por=request.user)

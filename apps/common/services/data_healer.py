...
class DataHealer:
    ...
    def heal_and_validate(cls, model_class: type[T], raw_data: dict[str, Any]) -> T:
        ...
        try:
            ...
            logger.warning(
                f"⚠️ Validación fallida para {model_class.__name__}. Iniciando 'curación' de datos."
            )
            ...
            if not missing_fields:
                ...
                logger.error(
                    f"❌ Error de validación no reparable en {model_class.__name__}: {str(e)}"
                )
                raise e
            ...
            for field in missing_fields:
                if field in cls.SMART_DEFAULTS:
                    ...
                    logger.info(
                        f"🩹 Curado campo faltante '{field}' con valor: {cls.SMART_DEFAULTS[field]}"
                    )
                else:
    ...
    @classmethod
    def heal_dict_only(
        cls, raw_data: dict[str, Any], model_class: type[BaseModel]
    ) -> dict[str, Any]:
        """

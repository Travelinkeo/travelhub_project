...
def sanitize_passenger_name(nombre: str) -> str:
    ...
    while candidate.endswith(")"):
        ...
        if (
            len(inner) >= 2
            and all(c in allowed_chars for c in upper_inner)
            and (
                (" " in inner)
                or "DE" in upper_inner
                or "DEL" in upper_inner
                or (inner.isalpha() and len(inner) >= 4)
            )
        ):
    ...
    if "/" in candidate:
        ...
        nombres = re.sub(
            r"(?:CIUDAD DE [A-ZÁÉÍÓÚÑ]{2,}(?:\s+[A-ZÁÉÍÓÚÑ]{2,})*)$", "", nombres
        ).strip()
...
def clean_currency(value):
    ...
    try:
        ...
        is_negative = str(value).strip().startswith("-")

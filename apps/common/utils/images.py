...
def get_image_as_base64(image_source):
    ...
    try:
        ...
        if isinstance(image_source, str) and (
            ...
            if "static.travelhub.cc" in image_source and not use_r2:
                logger.warning(
                    f"Evitando petición a {image_source} porque USE_R2 es False y el dominio no es accesible."
                )
                return None

            response = requests.get(image_source, timeout=2)
        ...
        # 2. Caso: Django ImageField / FileField
            ...
            try:
                ...
                logger.warning(
                    f"No se pudo abrir ImageField directamente: {e}. Reintentando via URL."
                )
...
def get_agencia_logo_b64(agencia, is_dark_bg=True):
    ...
    if is_dark_bg:
        ...
        logo_source = (
            getattr(agencia, "logo_dark", None)
            or getattr(agencia, "logo", None)
            or getattr(agencia, "logo_telegram_url", None)
        )
    else:
        # Para fondos claros, preferimos logo_light
        logo_source = (
            getattr(agencia, "logo_light", None)
            or getattr(agencia, "logo", None)
            or getattr(agencia, "logo_telegram_url", None)
        )

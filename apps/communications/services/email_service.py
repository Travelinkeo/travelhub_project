...
from .email_unified import send_custom_email
...
def send_ticket_to_client(venta_id, pdf_content, filename="boleto.pdf"):
    ...
    try:
        ...
        from_email = getattr(agencia, 'email_ventas', None) or getattr(agencia, 'email_principal', None) or settings.DEFAULT_FROM_EMAIL
        ...
        if resend_api_key:
            ...
            params = {
                ...
                "attachments": [
                    {
                        "filename": filename,
                        "content": list(pdf_content) if isinstance(pdf_content, bytes) else pdf_content
                    }
                ]
            }

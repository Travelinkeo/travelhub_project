...
from apps.communications.services.telegram_unified import TelegramNotificationService, enviar_alerta_telegram
...
def handle_urgent_notification(extracted_data: dict[str, Any]):
    ...
    if venta.google_calendar_event_id:
        try:
            from apps.bookings.services.calendar_service import update_calendar_event
...
def notificar_alerta_migratoria(check_instance):
    """
    Notifica a los agentes sobre alertas migratorias críticas.
    """
    mensaje = f"""⚠️ *ALERTA MIGRATORIA* ⚠️

Localizador: *{check_instance.localizador}*
Pasajero: {check_instance.pasajero_nombre}
Nivel: {check_instance.alert_level}

Resumen: {check_instance.summary}

Por favor, verifique los requisitos en el dashboard.
"""
    ...
    try:
        ...
        logger.error(f"Error notificando alerta migratoria por Telegram: {e}")

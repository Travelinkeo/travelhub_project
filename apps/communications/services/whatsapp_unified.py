...
def send_whatsapp_message(
    number: str, text: str, agencia=None, media_url=None, file_name=None
) -> dict[str, Any]:
    """
    ...
    if not agencia or not getattr(agencia, "subdominio_slug", None):
        ...
        return {
            "success": False,
            "error_message": "Se requiere una agencia con subdominio configurado",
        }
...
def enviar_whatsapp(telefono: str, mensaje: str, **kwargs) -> bool:
    ...
    Si se provee agencia, intenta usar la configuración de la agencia (Evolution API o Meta API).
    Si no, o si falla, cae de vuelta al Twilio global si está disponible.
    """
    ...
    if agencia:
        # 1. Intentar Evolution API (si tiene subdominio)
        if getattr(agencia, "subdominio_slug", None):
            try:
                ...
                if res.get("success"):
                    logger.info(
                        f"✅ WhatsApp enviado vía Evolution API para agencia {agencia.nombre} a {telefono}"
                    )
                    return True
            except Exception as e:
                logger.warning(
                    f"⚠️ Falló envío WhatsApp vía Evolution API: {e}. Cayendo a Meta API / Twilio."
                )

        # 2. Intentar Meta Cloud API (si es solo texto y tiene credenciales)
        if not media_url:
            try:
                res = enviar_mensaje_meta_api(telefono, mensaje, agencia=agencia)
                if res.get("success"):
                    logger.info(
                        f"✅ WhatsApp enviado vía Meta Cloud API para agencia {agencia.nombre} a {telefono}"
                    )
                    return True
            except Exception as e:
    ...
    try:
        ...
        logger.info(f"WhatsApp enviado a {telefono} vía Twilio: {message.sid}")
        return True

    except Exception as e:
        logger.error(f"Error enviando WhatsApp a {telefono} vía Twilio: {e}")
        return False
...
""".strip()
    ...
    return enviar_whatsapp(venta.cliente.telefono_principal, mensaje, agencia=agencia)
...
def enviar_whatsapp_confirmacion_pago(pago_venta) -> bool:
    ...
    saldo_msg = (
        ...
        if venta.saldo_pendiente <= 0
        else f"Saldo restante: {venta.moneda.simbolo if venta.moneda else ''}{venta.saldo_pendiente}"
    )
        ...
        try:
            ...
            try:
                datos = {
                    ...
                    "respuesta_bot": raw_resultado.get(
                        "respuesta_bot", "Entendido. Un agente te contactará pronto."
                    ),
                }
            ...
            cliente, _ = Cliente.objects.get_or_create(
                telefono_principal=telefono_limpio, defaults={"nombres": nombre_perfil}
            )
            ...
            from apps.common.tasks import send_whatsapp_meta_task

            send_whatsapp_meta_task.delay(telefono_cliente, resultado.respuesta_bot)

            # 7. Guardar Respuesta en Historial
            try:
                MensajeWhatsApp = apps.get_model("crm", "MensajeWhatsApp")

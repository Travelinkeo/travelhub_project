    ...
    def obtener_tasa_actual() -> Decimal | None:
        ...
        try:
            ...
            try:
                ...
                if ultima_tasa:
                    logger.warning(
                        f"🔄 Usando tasa histórica como fallback: {ultima_tasa.tasa_bsd_por_usd} (Fecha: {ultima_tasa.fecha})"
                    )
                    return ultima_tasa.tasa_bsd_por_usd
            except Exception as db_err:
                logger.critical(
                    f"❌ FALLO TOTAL: No se pudo recuperar tasa ni del API ni de la DB: {db_err}"
                )
    ...
    def actualizar_tasa_db(tasa: Decimal | None = None, fuente: str = "BCV API Official") -> bool:
        ...
        try:
            ...
            tasa_obj, created = TasaCambioBCV.objects.update_or_create(
                fecha=hoy, defaults={"tasa_bsd_por_usd": tasa, "fuente": fuente}
            )

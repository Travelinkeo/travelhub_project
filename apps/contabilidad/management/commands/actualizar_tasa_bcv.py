...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--fecha", type=str, help="Fecha en formato YYYY-MM-DD (default: hoy)")
        parser.add_argument(
            "--fuente", type=str, default="Manual", help="Fuente de la tasa (default: Manual)"
        )
    ...
    def handle(self, *args, **options):
        try:
            ...
            tasa_obj, created = TasaCambioBCV.objects.update_or_create(
                fecha=fecha_tasa, defaults={"tasa_bsd_por_usd": tasa, "fuente": fuente}
            )

            if created:
                self.stdout.write(
                    self.style.SUCCESS(f"[OK] Tasa BCV creada: {fecha_tasa} = {tasa} BSD/USD")
                )
            else:
                self.stdout.write(
                    self.style.SUCCESS(f"[OK] Tasa BCV actualizada: {fecha_tasa} = {tasa} BSD/USD")
                )

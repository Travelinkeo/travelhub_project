...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--todas", action="store_true", help="Muestra todas las tasas disponibles"
        )
    ...
    def handle(self, *args, **options):
        ...
        if not tasas:
            self.stdout.write(self.style.ERROR("[ERROR] No se pudieron obtener las tasas"))
            return
        ...
        if "oficial" in tasas:
            self.stdout.write(
                self.style.SUCCESS(f"  BCV OFICIAL:  {tasas['oficial']['price']:>10.2f} Bs/USD")
            )

        # Paralelo
        if "paralelo" in tasas:
            self.stdout.write(
                self.style.WARNING(f"  PARALELO:     {tasas['paralelo']['price']:>10.2f} Bs/USD")
            )

        # Bitcoin
        if "bitcoin" in tasas:
            self.stdout.write(
                self.style.HTTP_INFO(f"  BITCOIN:      {tasas['bitcoin']['price']:>10.2f} Bs/USD")
            )
        ...
        if dry_run:
            ...
            if resultados.get("oficial"):
                self.stdout.write(self.style.SUCCESS("[OK] Tasa BCV guardada correctamente"))
            else:
                self.stdout.write(self.style.ERROR("[ERROR] No se pudo guardar la tasa BCV"))

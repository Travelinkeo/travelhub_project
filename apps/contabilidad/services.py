...
class ContabilidadService:
    ...
    def generar_asiento_desde_factura(factura: Factura) -> AsientoContable:
        ...
        try:
            ...
            # 2. Obtener o crear asiento contable
    ...
    def _generar_lineas_intermediacion(
        asiento: AsientoContable, factura: Factura, tasa: Decimal, linea_num: int
    ) -> int:
        """
            ...
            if abs(diferencial_bsd) > Decimal("0.01"):  # Tolerancia
                if diferencial_bsd > 0:
                    ...
                    if nota_debito:
                        ...
                        logger.info(
                            f"Nota de Débito {nota_debito.numero_nota_debito} generada: IVA {nota_debito.monto_iva_bsd} BSD"
                        )

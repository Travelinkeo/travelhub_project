...
def parse_latin_decimal(val_str: str) -> Decimal:
    """
    Convierte formato numérico con punto de mil y coma decimal (ej. 1.990,74 o 38,43 o -3.217,63) a Decimal
    """
    if not val_str or val_str.strip() in ["-", ""]:
        return Decimal("0.00")
    clean = val_str.replace("%", "").strip()
    # Reemplazar puntos de miles por nada y comas por puntos
...
def parse_date(date_str: str) -> str | None:
    """Convierte fecha D/M/YYYY o M/D/YYYY a YYYY-MM-DD"""
    if not date_str:
        return None
    ...
    if len(parts) == 3:
        try:
            ...
            # Intercambiar si el mes viene primero o el día
            if month > 12:
                day, month = month, day
...
class MyDestinyReportParser(BaseSupplierReportParser):
    """
    Parser especializado para reportes de ventas de MY DESTINY (Agencia de Viajes My Destiny 1425 C.A.).
    """
    ...
    def parse(self) -> dict[str, Any]:
        ...
        # 1. Extraer Saldo Anterior y Diferencia/Total
        ...
        # 2. Expresión Regular para Filas de Boletos MY DESTINY
        # Ejemplo: 3/2/2026 PTYS3650 TRAVELINKEO HOYOS VELARDE/HECTOR FABIO 7420307424814 AVIOR 9V 38,43 157,56 - - 195,99 10,00 3,0% 1,15 204,84
        ...
        # Ajustar fechas desde/hasta según los ítems si no estaban definidas
        if result["items"]:

...
class CotizacionAdmin(SaaSAdminMixin, admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("numero_cotizacion", "uuid", "estado", "consultor")}),
        (
            "Magic IA (Cotizador)",
            {
                "fields": (
        ...
        ("Detalles del Viaje", {"fields": ("destino", "fecha_emision", "fecha_validez")}),
        ("Financiero", {"fields": ("total_cotizado",)}),
        ("Términos y PDF", {"fields": ("condiciones_comerciales", "notas_internas", "ver_pdf")}),
    )
    ...
    def ver_link_publico(self, obj):
        if obj.uuid:
            ...
            return format_html(
                '<a href="{}" target="_blank">🔗 Link Público para WhatsApp</a>', url
            )
        return "-"
    ...
    def generar_pdf_action(self, request, queryset):
        for cotizacion in queryset:
            try:
                ...
                self.message_user(
                    request, f"PDF generado para la cotización {cotizacion.numero_cotizacion}"
                )
            except Exception as e:
                self.message_user(
                    request,
                    f"Error al generar PDF para {cotizacion.numero_cotizacion}: {e}",
                    level="error",
                )

...
class FreelancerService:
    ...
    def calculate_commission(venta) -> None:
        ...
        try:
            ...
            logger.error(f"Error buscando FreelancerProfile para el usuario {venta.creado_por}: {e}")
            return

        if not freelancer:
            logger.debug(f"El creador de la venta {venta.creado_por.username} no es un freelancer activo.")
            return

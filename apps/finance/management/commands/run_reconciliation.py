...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('reporte_id', type=str, help='UUID del ReporteReconciliacion a procesar')
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(self.style.WARNING(f'Discrepancias: {resumen.get("discrepancias", 0)}'))
            self.stdout.write(self.style.WARNING(f'Huérfanos reporte: {resumen.get("huerfanos_reporte", 0)}'))
            self.stdout.write(self.style.WARNING(f'Huérfanos local: {resumen.get("huerfanos_local", 0)}'))

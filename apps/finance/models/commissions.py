...
class ReglaComision(AgenciaMixin, SoftDeleteModel, models.Model):
    """
    ...
    tipo_calculo = models.CharField(
        ...
        default=TipoCalculo.PORCENTAJE_UTILIDAD,
    )
    valor = models.DecimalField(
        _("Valor (Propina/%)"),
        max_digits=12,
        decimal_places=2,
        help_text=_("Monto en $ o % según el tipo."),
    )
...
class ComisionVenta(AgenciaMixin, SoftDeleteModel, models.Model):
    """
    ...
    regla_aplicada = models.ForeignKey(
        ReglaComision, on_delete=models.SET_NULL, null=True, blank=True
    )
    ...
    monto_base_calculo = models.DecimalField(
        ...
        max_digits=12,
        decimal_places=2,
        help_text=_("Total Venta o Utilidad usada para el cálculo."),
    )
    monto_comision = models.DecimalField(_("Monto Comisión"), max_digits=12, decimal_places=2)

    estado = models.CharField(
        _("Estado"), max_length=3, choices=EstadoComision.choices, default=EstadoComision.PENDIENTE
    )
...
class LiquidacionAgente(AgenciaMixin, SoftDeleteModel, models.Model):
    """
    ...
    # agencia la provee AgenciaMixin
    agente = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="liquidaciones_mensuales"
    )
    ...
    total_comisiones = models.DecimalField(
        _("Total Comisiones"), max_digits=12, decimal_places=2, default=0
    )

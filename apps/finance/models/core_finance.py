...
def get_audit_helpers():
    from apps.bookings.models import _crear_audit_log, _calcular_diff, AuditLog
    return _crear_audit_log, _calcular_diff, AuditLog
...
class Factura(models.Model):
    ...
    cliente = models.ForeignKey(Cliente, on_delete=models.PROTECT, verbose_name=_("Cliente"), blank=True, null=True)
...
def audit_post_save_factura(sender, instance, created, **kwargs):
    try:
        _crear_audit_log, _calcular_diff, AuditLog = get_audit_helpers()
        
        accion = AuditLog.Accion.CREATE if created else 'UPDATE'
        datos_previos = None
        datos_nuevos = None
        descripcion = f"Factura {'creada' if created else 'actualizada'}: {instance.numero_factura}"
        ...
        logger.error(f"Error en auditoría de Factura {instance.pk}: {e}")

@receiver(post_delete, sender=Factura)
def audit_delete_factura(sender, instance, **kwargs):
    try:
        _crear_audit_log, _, AuditLog = get_audit_helpers()
        _crear_audit_log(
            modelo='Factura', 
            object_id=instance.pk, 
            accion=AuditLog.Accion.DELETE, 
            venta=instance.venta_asociada, 
            descripcion=f"Factura eliminada: {instance.numero_factura}"
        )
    except Exception as e:
        logger.error(f"Error en auditoría de eliminación de Factura {instance.pk}: {e}")

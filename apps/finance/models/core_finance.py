...
class Factura(SoftDeleteModel, AgenciaMixin, models.Model):
    ...
    es_sujeto_pasivo_especial = models.BooleanField(_("Es Sujeto Pasivo Especial"), default=False, help_text=_("Determina obligaciones como agente de percepción IGTF"))
    esta_inscrita_rtn = models.BooleanField(_("Inscrita en RTN"), default=False, help_text=_("Registro Turístico Nacional"))
    
    cliente_es_residente = models.BooleanField(_("Cliente es Residente"), default=True, help_text=_("Determina si aplica exportación de servicios (alícuota 0%)"))
    cliente_identificacion = models.CharField(_("Identificación Cliente"), max_length=50, blank=True, help_text=_("Cédula, RIF o Pasaporte"))
    ...
    tipo_operacion = models.CharField(_("Tipo de Operación"), max_length=20, choices=TipoOperacion.choices, default=TipoOperacion.VENTA_PROPIA)
    
    class MonedaOperacion(models.TextChoices):
        BOLIVAR = 'BOLIVAR', _('Bolívar')
        DIVISA = 'DIVISA', _('Divisa')
    moneda_operacion = models.CharField(_("Moneda de Operación"), max_length=10, choices=MonedaOperacion.choices, default=MonedaOperacion.DIVISA)
    tasa_cambio_bcv = models.DecimalField(_("Tasa de Cambio BCV"), max_digits=12, decimal_places=4, blank=True, null=True, help_text=_("Tasa oficial BCV del día de la operación"))
    ...
    modalidad_emision = models.CharField(_("Modalidad de Emisión"), max_length=20, choices=ModalidadEmision.choices, default=ModalidadEmision.DIGITAL)
    ...
    def clean(self):
        ...
        if getattr(self, 'tipo_operacion', None) == self.TipoOperacion.INTERMEDIACION:
            if not getattr(self, 'tercero_rif', None) or not getattr(self, 'tercero_razon_social', None):
                raise ValidationError({
                    'tercero_rif': _('El RIF del tercero es obligatorio para operaciones de intermediación.'),
                    'tercero_razon_social': _('La razón social del tercero es obligatoria para operaciones de intermediación.')
                })
        
        if getattr(self, 'moneda_operacion', None) == self.MonedaOperacion.DIVISA and not getattr(self, 'tasa_cambio_bcv', None):
            raise ValidationError({
                'tasa_cambio_bcv': _('La tasa de cambio oficial BCV es obligatoria para operaciones pactadas en divisas.')
            })
    ...
    def calcular_impuestos_venezuela(self):
        ...
        for item in items:
            ...
            if tipo_servicio == 'TRANSPORTE_AEREO_NACIONAL' or not es_gravado:
                ...
                monto_iva += (subtotal_item * (alicuota / Decimal('100.00'))).quantize(Decimal('0.01'))
        ...
        if self.es_sujeto_pasivo_especial and self.moneda_operacion == 'DIVISA':
            base_igtf = self.subtotal_base_gravada + self.monto_iva_16 + self.subtotal_exento + self.subtotal_exportacion
            self.monto_igtf = (base_igtf * Decimal('0.03')).quantize(Decimal('0.01'))
        else:
            self.monto_igtf = Decimal('0.00')

        self.subtotal = (self.subtotal_base_gravada + self.subtotal_exento + self.subtotal_exportacion).quantize(Decimal('0.01'))
        self.monto_total = (self.subtotal + self.monto_iva_16 + self.monto_igtf).quantize(Decimal('0.01'))
        
        if self.estado == self.EstadoFactura.BORRADOR:
            self.saldo_pendiente = self.monto_total
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.numero_factura:
            ...
            for attempt in range(max_retries):
                with transaction.atomic():
                    daily_count = self.__class__.objects.filter(
                        fecha_emision=self.fecha_emision
                    ).count()
                    candidate = f"{prefix}-{daily_count + 1:04d}"
                    if not self.__class__.objects.filter(numero_factura=candidate).exists():
        ...
        if self.cliente and not self.cliente_rif:
            try:
                ...
                self.cliente_rif = getattr(self.cliente, 'numero_documento', '') or getattr(self.cliente, 'cedula_identidad', '') or ''
        ...
        if self.subtotal_base_gravada or self.subtotal_exento or self.subtotal_exportacion or self.monto_iva_16 or self.monto_igtf:
...
class ItemFactura(SoftDeleteModel, AgenciaMixin, models.Model):
    ...
    tipo_impuesto = models.CharField(_("Tipo Impuesto"), max_length=2, choices=TipoImpuesto.choices, default=TipoImpuesto.IVA_25)
    ...
    tipo_servicio = models.CharField(_("Tipo de Servicio"), max_length=30, choices=TipoServicio.choices, default=TipoServicio.ALOJAMIENTO_Y_OTROS_GRAVADOS)
    es_gravado = models.BooleanField(_("Es Gravado"), default=True)
    alicuota_iva = models.DecimalField(_("Alícuota IVA"), max_digits=5, decimal_places=2, default=Decimal('25.00'))
    ...
    def clean(self):
        ...
        if getattr(self, 'tipo_servicio', None) == self.TipoServicio.TRANSPORTE_AEREO_NACIONAL:
            ...
            if not getattr(self, 'nombre_pasajero', None):
                errors['nombre_pasajero'] = _('El nombre del pasajero es obligatorio para boletos aéreos.')
            if not getattr(self, 'numero_boleto', None):
                errors['numero_boleto'] = _('El número de boleto es obligatorio para boletos aéreos.')
...
class DocumentoExportacion(models.Model):
    ...
    factura = models.ForeignKey(Factura, related_name='documentos_exportacion', on_delete=models.CASCADE, null=True, blank=True)
    ...
    archivo = models.FileField(_("Archivo"), upload_to='documentos_exportacion/%Y/%m/', blank=True, null=True)
...
class PropuestaTransaccionIA(SoftDeleteModel, AgenciaMixin, models.Model):
    """
    ...
    modulo_objetivo = models.CharField(_("Módulo Objetivo"), max_length=50) # 'CONTABILIDAD', 'FINANZAS', 'RECONCILIACION'
    accion_tipo = models.CharField(_("Tipo de Acción"), max_length=50)     # 'CREAR_ASIENTO', 'CONCILIAR_BOLETO', 'REGISTRAR_GASTO'
    
    # Carga útil estructurada de la transacción
    payload_datos = models.JSONField(_("Payload de Datos"), help_text=_("Datos exactos a procesar determinísticamente al aprobar"))
    ia_justificacion = models.TextField(_("Justificación de la IA"), help_text=_("Explicación y razonamiento en lenguaje natural de la IA"))
    
    # Estados de Aprobación
    estado = models.CharField(_("Estado"), max_length=3, choices=EstadoPropuesta.choices, default=EstadoPropuesta.PENDIENTE)

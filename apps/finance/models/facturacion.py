...
"""
Modelo consolidado de facturación para Venezuela (Migrado desde core)
...
class FacturaConsolidada(AgenciaMixin, models.Model):
    ...
    numero_factura = models.CharField(
        _("Número de Factura"), max_length=50, unique=True, blank=True
    )
    ...
    cliente_identificacion = models.CharField(
        _("Identificación Cliente"), max_length=50, help_text=_("Cédula, RIF o Pasaporte")
    )
    ...
    subtotal = models.DecimalField(
        _("Subtotal"), max_digits=12, decimal_places=2, default=0, editable=False
    )
    monto_total = models.DecimalField(
        _("Monto Total"), max_digits=12, decimal_places=2, default=0, editable=False
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.numero_factura:
            self.numero_factura = generar_numero_factura_atomico(
                FacturaConsolidada, self.fecha_emision
            )
    ...
    def recalcular_totales(self):
        """
        [DEPRECATED] Usar apps.finance.services.facturacion_service.FacturacionService.recalculate_invoice_totals
        """
...
class ItemFacturaConsolidada(AgenciaMixin, models.Model):
    ...
    class TipoServicio(models.TextChoices):
        ...
        TRANSPORTE_AEREO_INTERNACIONAL = (
            "TRANSPORTE_AEREO_INTERNACIONAL",
            _("Transporte Aéreo Internacional"),
        )
    ...
    es_gravado = models.BooleanField(
        _("Es Gravado"), default=True, help_text=_("Si aplica IVA al item")
    )
    ...
    numero_boleto = models.CharField(
        _("Número Boleto"), max_length=50, blank=True, help_text=_("Número del boleto de pasaje")
    )
    itinerario = models.TextField(
        _("Itinerario"), blank=True, help_text=_("Ruta del viaje (ej: CCS-MIA-CCS)")
    )
    codigo_aerolinea = models.CharField(
        _("Código Aerolínea"), max_length=10, blank=True, help_text=_("Código IATA de la aerolínea")
    )

...
class ReporteReconciliacion(AgenciaMixin, SoftDeleteModel, models.Model):
    """
    ...
    datos_extraidos = models.JSONField(
        null=True, blank=True, help_text="Raw data extraída por la IA"
    )
    resumen_conciliacion = models.JSONField(
        null=True, blank=True, help_text="Resumen de match/mismatch"
    )
...
class LineaReporteReconciliacion(AgenciaMixin, SoftDeleteModel, models.Model):
    """
    ...
    reporte = models.ForeignKey(
        ReporteReconciliacion, on_delete=models.CASCADE, related_name="lineas"
    )
    numero_boleto_reportado = models.CharField(
        _("Número de Boleto (Proveedor)"), max_length=150, db_index=True
    )

    tarifa_base_cobrada = models.DecimalField(
        _("Tarifa Base Cobrada"), max_digits=12, decimal_places=2, default=0
    )
    impuestos_cobrados = models.DecimalField(
        _("Impuestos Cobrados"), max_digits=12, decimal_places=2, default=0
    )
    comision_cedida = models.DecimalField(
        _("Comisión Cedida (-/+)"), max_digits=12, decimal_places=2, default=0
    )
    total_cobrado = models.DecimalField(
        _("Total Cobrado/Liquidado"), max_digits=12, decimal_places=2, default=0
    )
...
class ConciliacionBoleto(AgenciaMixin, SoftDeleteModel, models.Model):
    """
    ...
    reporte = models.ForeignKey(
        ReporteReconciliacion, on_delete=models.CASCADE, related_name="conciliaciones"
    )

    # Ambos pueden ser nulos si es huérfano de algún lado
    linea_reporte = models.OneToOneField(
        LineaReporteReconciliacion, on_delete=models.SET_NULL, null=True, blank=True
    )
    boleto_local = models.OneToOneField(
        "bookings.BoletoImportado", on_delete=models.SET_NULL, null=True, blank=True
    )
    ...
    sugerencia_asiento = models.ForeignKey(
    ...
    ia_razonamiento = models.TextField(
        blank=True, null=True, help_text="Explicación de la IA sobre el match difuso o discrepancia"
    )

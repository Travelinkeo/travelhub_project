...
class FinancialAnalyticsService:
    ...
    def get_monthly_profitability():
        ...
        # Agrupar por mes
        stats = (
            ItemVenta.objects.annotate(month=TruncMonth("venta__fecha_venta"))
            .values("month")
            .annotate(
    ...
    def get_profit_by_category():
        ...
        """
        stats = (
            ItemVenta.objects.values("producto_servicio__nombre")
            .annotate(
                margen=Sum(
        ...
        return [
            {"name": s["producto_servicio__nombre"], "value": float(s["margen"])}
            for s in stats
            if s["margen"]
        ]

...
class CommissionService:
    ...
    def calcular_comision_venta(cls, venta_id: int) -> bool:
        ...
        try:
            ...
            if not agente:
                logger.warning(
                    f"Venta {venta_id} no tiene agente asignado. No se calcula comisión."
                )
                return False

            # 2. Localizar Regla de Comisión Activa para el Agente
            regla = ReglaComision.objects.filter(
                agencia=venta.agencia, agente=agente, activo=True
            ).first()

            if not regla:
                logger.warning(
                    f"Agente {agente.username} no tiene una Regla de Comisión configurada."
                )
                return False
            ...
            costo_proveedor = sum(
                ...
                for i in venta.items_venta.only("costo_neto_proveedor", "cantidad").all()
            )
            ...
            if regla.tipo_calculo == ReglaComision.TipoCalculo.PORCENTAJE_UTILIDAD:
                ...
                if base_calculo > 0:
                    monto_comision = (base_calculo * (regla.valor / Decimal("100"))).quantize(
                        Decimal("0.01")
                    )
            ...
            logger.info(
                f"✅ Comisión de ${monto_comision} calculada para Agente {agente.username} (Venta {venta.localizador})"
            )
            return True

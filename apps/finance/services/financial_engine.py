...
class FinancialEngine:
    ...
    def calculate_ticket_amounts(data, boleto_obj=None):
        ...
        if boleto_obj and (boleto_obj.total_boleto or 0) > 0:
            ...
            logger.info(
                f"💰 Usando montos MANUALES del Boleto {boleto_obj.pk}: Total {monto_total}"
            )
        else:
            # Intentar extraer de múltiples alias posibles del parser
            # Aseguramos que siempre pasen por clean_currency
            monto_base = clean_currency(
                data.get("tarifa") or data.get("fare_amount") or data.get("TARIFA_IMPORTE") or 0
            )
            monto_total = clean_currency(
                data.get("total") or data.get("total_amount") or data.get("TOTAL_IMPORTE") or 0
            )

            # Desglose de impuestos (IA suele enviar 'tax_details' o 'tarifas.taxes_breakdown')
            tax_data = (
                data.get("tax_details") or data.get("tarifas", {}).get("taxes_breakdown") or {}
            )

            monto_iva_yn = clean_currency(
                tax_data.get("iva") or tax_data.get("iva_yn") or tax_data.get("VAT") or 0
            )
            monto_inatur = clean_currency(
                tax_data.get("inatur") or tax_data.get("tourism_tax") or 0
            )
            monto_otros_tax = clean_currency(
                tax_data.get("other_taxes") or tax_data.get("otros") or 0
            )

            # Si no hay desglose, pero hay un monto total de impuestos
            if monto_iva_yn == 0 and monto_inatur == 0 and monto_otros_tax == 0:
                impuestos_total = clean_currency(
                    data.get("taxes_amount") or data.get("impuestos") or 0
                )
        ...
        moneda_obj, _ = Moneda.objects.get_or_create(
            codigo_iso=codigo_moneda, defaults={"nombre": codigo_moneda}
        )
    ...
    def calculate_margins(monto_total, data, proveedor_id=None):
        ...
        if proveedor_id:
            try:
                ...
                tarifario = (
                    TarifarioProveedor.objects.filter(
                    ...
                    if nombre_air:
                        ...
                        if air_obj:
                            override = ComisionOverrideAerolinea.objects.filter(
                                tarifario=tarifario, aerolinea=air_obj
                            ).first()

                            if override:
                                logger.info(
                                    f"🎯 Aplicando OVERRIDE de comisión: {override.comision_porcentaje}% para {air_obj.nombre}"
                                )

...
class InvoiceMatchingService:
    ...
    def auto_match_invoice(factura_proveedor_id):
        ...
        logger.info(
            f"🔍 [MATCHING] Iniciando cotejo para factura {numero_factura} de {factura.proveedor_nombre}"
        )
        ...
        if proveedor_nombre_ia:
            ...
            proveedor_erp = Proveedor.objects.filter(
                agencia_id=agencia_id, nombre__icontains=busqueda
            ).first()
        ...
        for c in candidatos:
            ...
            match_pnr = (pnr_erp and pnr_erp in raw_data_str) or (
                loc_venta and loc_venta in raw_data_str
            )
            ...
            if match_pnr:
                score += 100
            if proveedor_erp and c.proveedor_servicio == proveedor_erp:
                score += 30
        ...
        if len(potential_matches) == 1 or (
            len(potential_matches) > 1 and potential_matches[0][1] > potential_matches[1][1] + 20
        ):
            ...
            logger.info(
                f"🎯 [MATCH SUCCESS] Factura {numero_factura} conciliada con Item {match.id_item_venta} (Score: {score})"
            )
            return True
            ...
            if not factura.datos_json:
                factura.datos_json = {}
            factura.datos_json["posibles_candidatos"] = [
                m[0].id_item_venta for m in potential_matches[:3]
            ]
            factura.save()
            logger.warning(
                f"⚠️ [MATCH AMBIGUOUS] Múltiples candidatos para factura {numero_factura}"
            )
            return False
            ...
            factura.save()
            logger.info(
                f"❓ [MATCH NOT FOUND] No se encontró contraparte automática para factura {numero_factura}"
            )
            return False

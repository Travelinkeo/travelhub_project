...
class ReporteRetencionesService:
    ...
    def reporte_periodo(fecha_inicio, fecha_fin, estado=None):
        ...
        por_tipo = {}
        agg_data = retenciones.values("tipo_operacion").annotate(
            cantidad=Count("id_retencion"),
            base_imponible=Sum("base_imponible"),
            monto_retenido=Sum("monto_retenido"),
        )
        tipo_map = dict(RetencionISLR.TipoOperacion.choices)
        for row in agg_data:
            tipo_name = tipo_map.get(row["tipo_operacion"], row["tipo_operacion"])
            por_tipo[tipo_name] = {
                "cantidad": row["cantidad"],
                "base_imponible": row["base_imponible"] or Decimal("0.00"),
                "monto_retenido": row["monto_retenido"] or Decimal("0.00"),
            }
            ...
            detalle.append(
                {
                    ...
                    "cliente": f"{retencion.cliente.nombres} {retencion.cliente.apellidos}".strip()
                    or retencion.cliente.nombre_empresa,

...
class LineaCobro(BaseModel):
    ...
    fare_cobrado: float = Field(
        description="Monto de la tarifa base (Fare/Net) que cobró el proveedor. 0.0 si es nulo."
    )
    taxes_cobrados: float = Field(
        description="Monto de los impuestos (Taxes) cobrados por el proveedor."
    )
    comision_cedida: float = Field(
        description="Comisión a favor de la agencia cedida por el proveedor en este boleto. Si es un cobro extra, será negativo."
    )
    total_cobrado: float = Field(
        description="Monto final liquidado y cobrado por el proveedor por este ticket (Suele ser Fare + Taxes - Comm)."
    )
    raw_reference: str = Field(
        description="Breve texto original de la línea para auditoría visual rápida."
    )


class ReporteLiquidacionSchema(BaseModel):
    """ReporteLiquidacionSchema."""

    proveedor_detectado: str = Field(
        description="Nombre del proveedor o consolidador (Ej: SABRE, KIU, AMADEUS, TICKET_CONSOLIDATOR)."
    )
    lineas_cobradas: list[LineaCobro] = Field(
        description="Array con todas las filas de boletos cobrados extraídas del reporte."
    )
...
class SmartReconciliationService:
    ...
    def procesar_reporte(cls, reporte_id: str) -> None:
        ...
        try:
            ...
            if (
                resumen.get("discrepancias", 0) > 0
                or resumen.get("huerfanos_reporte", 0) > 0
                or resumen.get("huerfanos_local", 0) > 0
            ):
    ...
    def _extraer_datos_archivo(cls, reporte: ReporteReconciliacion) -> dict[str, Any]:
        ...
        if file_path.lower().endswith((".pdf", ".eml", ".txt")):
            ...
            SupplierReportParser = import_string(
                "apps.automation.parsers.supplier_report_parser.SupplierReportParser"
            )
    ...
    def _mapear_columnas_df_con_ia(cls, df: pd.DataFrame, proveedor_hint: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {"proveedor_nombre": proveedor_hint, "items": items}
        except Exception as e:
            logger.error(f"Fallo mapeando columnas con IA: {e}")
            # Fallback: intentar match directo si los nombres ya coinciden
            return {"proveedor_nombre": proveedor_hint, "items": df.to_dict(orient="records")}

    @classmethod
    @transaction.atomic
    def _guardar_lineas_extraidas(
        cls, reporte: ReporteReconciliacion, datos_ia: dict[str, Any]
    ) -> None:
        ...
        reporte.lineas.all().hard_delete()
    ...
    def _ejecutar_cruce_conciliacion(cls, reporte: ReporteReconciliacion) -> dict[str, Any]:
        ...
        ventas_locales = list(
            query_local.annotate(
                id_boleto=F("id_boleto_importado"),
                pnr=F("localizador_pnr"),
                pasajero_nombre_completo=F("nombre_pasajero_completo"),
            ).values(
                "id_boleto",
                "numero_boleto",
                "pnr",
        ...
        if pendientes_reporte.exists() and pendientes_local:
            logger.info(
                f"🤖 Ejecutando Cruce Fuzzy IA para {pendientes_reporte.count()} líneas pendientes."
            )
            ...
            for chunk in chunk_list(list(pendientes_reporte), 20):
                cls._procesar_lote_fuzzy_ia(
                    reporte, chunk, pendientes_local, boletos_asignados, resumen
                )
    ...
    def _crear_conciliacion(cls, reporte, linea, match_data, resumen):
        ...
        conciliacion = ConciliacionBoleto.objects.create(
            ...
            estado=estado,
            diferencia_tarifa=linea.tarifa_base_cobrada
            - Decimal(str(match_data["tarifa_base"] or 0)),
            diferencia_impuestos=linea.impuestos_cobrados
            - Decimal(str(match_data["impuestos_total_calculado"] or 0)),
    ...
    @classmethod
    def _procesar_lote_fuzzy_ia(
        cls, reporte, chunk_lineas, pendientes_local, boletos_asignados, resumen
    ):
        """Usa Gemini para encontrar matches semánticos en un lote de registros"""
        from django.utils.module_loading import import_string

        ai_engine = import_string("apps.automation.services.ai_engine.ai_engine")
        RECONCILIATION_SYSTEM_PROMPT = import_string(
            "apps.automation.services.prompts.RECONCILIATION_SYSTEM_PROMPT"
        )
        ConciliacionLoteSchema = import_string("core.models.ai_schemas.ConciliacionLoteSchema")
        ...
        try:
            ...
            for match in resultado.get("matches", []):
                ...
                linea_obj = next(
                    (item for item in chunk_lineas if item.id_linea == int(linea_id)), None
                )
                ...
                if linea_obj and venta and venta["id_boleto"] not in boletos_asignados:
                    ...
                    c = ConciliacionBoleto.objects.filter(
                        reporte=reporte, linea_reporte=linea_obj
                    ).last()
                    if c:
    ...
    @classmethod
    def _get_cuenta_contable(
        cls, agencia, config_key: str, fallback_codigo: str, tipo_cuenta_fallback: str
    ):
        """
        ...
        try:
            ...
            if not cuenta:
                logger.error(
                    f"¡CRÍTICO! No se encontró ninguna cuenta de tipo {tipo_cuenta_fallback} para la agencia {agencia.nombre}."
                )
            return cuenta
    ...
    def proponer_asiento_ajuste(cls, conciliacion: ConciliacionBoleto) -> None:
        ...
        if (
            ...
            or conciliacion.diferencia_total == 0
        ):
            return
        ...
        try:
            ...
            tipo = (
                ...
                else "Ganancia (Ahorro Proveedor)"
            )
            monto_abs = abs(conciliacion.diferencia_total)
            boleto = (
                conciliacion.linea_reporte.numero_boleto_reportado
                if (
                    conciliacion.linea_reporte
                    and conciliacion.linea_reporte.numero_boleto_reportado
                )
                else str(conciliacion.id_conciliacion)
            )

            # Obtener tasa BCV actual para BSD (VEN-NIF)
            from django.utils.module_loading import import_string

            ContabilidadService = import_string("apps.contabilidad.services.ContabilidadService")
            ...
            if conciliacion.diferencia_total > 0:
                ...
                if not cuenta_ajuste or not cuenta_proveedor:
                    raise ValueError(
                        "Faltan cuentas contables críticas para generar el asiento de pérdida."
                    )
                ...
                if not cuenta_ajuste or not cuenta_proveedor:
                    raise ValueError(
                        "Faltan cuentas contables críticas para generar el asiento de ganancia."
                    )
            ...
            logger.error(
                f"Fallo sugiriendo asiento para conciliación {conciliacion.id_conciliacion}: {str(e)}"
            )

    @classmethod
    def _buscar_boleto_difuso_con_ia(
        cls, linea: LineaReporteReconciliacion
    ) -> tuple[BoletoImportado | None, str | None]:
        """
        ...
        try:
            res = ai_engine.call_gemini(prompt=prompt, response_schema=MatchResult, temperature=0.0)

...
CiudadSerializer = import_string("core.serializers.CiudadSerializer")
MonedaSerializer = import_string("core.serializers.MonedaSerializer")
PaisSerializer = import_string("core.serializers.PaisSerializer")
TipoCambioSerializer = import_string("core.serializers.TipoCambioSerializer")

# Billing Analytics Views
get_churn_rate = import_string("core.views.billing_analytics_views.get_churn_rate")
get_conversion_funnel = import_string("core.views.billing_analytics_views.get_conversion_funnel")
get_growth_metrics = import_string("core.views.billing_analytics_views.get_growth_metrics")
get_mrr = import_string("core.views.billing_analytics_views.get_mrr")
get_usage_metrics = import_string("core.views.billing_analytics_views.get_usage_metrics")

# Billing Dashboard Views
get_invoices = import_string("core.views.billing_dashboard_views.get_invoices")
get_payment_method = import_string("core.views.billing_dashboard_views.get_payment_method")
get_usage_stats = import_string("core.views.billing_dashboard_views.get_usage_stats")

# Billing Plan Change Views
change_plan = import_string("core.views.billing_plan_change_views.change_plan")
downgrade_to_free = import_string("core.views.billing_plan_change_views.downgrade_to_free")
preview_plan_change = import_string("core.views.billing_plan_change_views.preview_plan_change")

# Billing Success Views
billing_cancel = import_string("core.views.billing_success_views.billing_cancel")
billing_success = import_string("core.views.billing_success_views.billing_success")

# Billing Views
cancel_subscription = import_string("core.views.billing_views.cancel_subscription")
create_checkout_session = import_string("core.views.billing_views.create_checkout_session")
create_portal_session = import_string("core.views.billing_views.create_portal_session")
get_current_subscription = import_string("core.views.billing_views.get_current_subscription")
get_plans = import_string("core.views.billing_views.get_plans")
stripe_webhook = import_string("core.views.billing_views.stripe_webhook")

# Reportes Views
balance_comprobacion = import_string("core.views.reportes_views.balance_comprobacion")
estado_resultados = import_string("core.views.reportes_views.estado_resultados")
exportar_excel = import_string("core.views.reportes_views.exportar_excel")
...
router.register(
    r"api/reconciliacion",
    api_reconciliacion_views.ReporteReconciliacionViewSet,
    basename="api-reconciliacion",
)
...
urlpatterns = [
    ...
    # Binance Pay
    path(
        "pago/binance/crear/<int:factura_id>/",
        payment_views.BinanceOrderCreateView.as_view(),
        name="binance_crear_orden",
    ),
    path(
        "pago/binance/webhook/", payment_views.BinanceWebhookView.as_view(), name="binance_webhook"
    ),  # Legacy
    path(
    ...
    path(
        "ai-chat/proposals/",
        ai_views.AIAccountingProposalsPartialView.as_view(),
        name="ai_proposals_partial",
    ),
    path(
        "ai-chat/proposals/<uuid:pk>/<str:action>/",
        ai_views.AIAccountingResolveProposalHTMXView.as_view(),
        name="ai_proposal_resolve_htmx",
    ),
    # Conciliación Contable Inteligente (Fase 21) Reescritura HTMX Pura
    path(
        "reconciliacion/dashboard/",
        reconciliacion_views.ReconciliationDashboardHTMXView.as_view(),
        name="reconciliacion_dashboard_htmx",
    ),
    path(
        "reconciliacion/subir/",
        reconciliacion_views.ReporteReconciliacionCreateView.as_view(),
        name="reconciliacion_create",
    ),
    path(
        "reconciliacion/detalle/<uuid:pk>/",
        reconciliacion_views.ReporteReconciliacionDetailView.as_view(),
        name="reconciliacion_detail",
    ),
    path(
        "reconciliacion/procesar-ia/<uuid:pk>/",
        reconciliacion_views.ProcessReconciliacionHTMXView.as_view(),
        name="reconciliacion_process",
    ),
    path(
        "reconciliacion/discrepancia/<int:pk>/analizar-ia/",
        ai_views.ResolveDiscrepancyAIView.as_view(),
        name="resolve_discrepancy_ai",
    ),
    # NUEVO FLUJO CABINA DE PILOTAJE (HTMX + CELERY)
    path(
        "reconciliacion/upload/",
        reconciliation_ui.ReconciliationUploadView.as_view(),
        name="reconciliacion_upload",
    ),
    path(
        "reconciliacion/process-upload/",
        reconciliation_ui.process_reconciliation_upload_htmx,
        name="reconciliacion_process_upload",
    ),
    path(
        "reconciliacion/task-status/<str:task_id>/",
        reconciliation_ui.reconciliation_task_status_htmx,
        name="reconciliacion_task_status",
    ),
    path(
        "reconciliacion/results/<uuid:pk>/",
        audit_ui.ReconciliationResultsView.as_view(),
        name="reconciliacion_results",
    ),
    path(
        "reconciliacion/audit/approve/<int:conciliacion_id>/",
        audit_ui.approve_adjustment_htmx,
        name="audit_approve_adjustment",
    ),
    path(
        "reconciliacion/report/pdf/<uuid:pk>/",
        report_ui.download_reconciliation_report_view,
        name="reconciliacion_report_pdf",
    ),
    path(
        "reconciliacion/report/email/<uuid:pk>/",
        report_ui.send_reconciliation_report_email_htmx,
        name="reconciliacion_report_email",
    ),
    path(
        "tax-refund/",
        tax_refund_views.TaxRefundDashboardView.as_view(),
        name="tax_refund_dashboard",
    ),
    path(
        "tax-refund/iniciar/<uuid:reclamo_id>/",
        tax_refund_views.IniciarTramiteRefundView.as_view(),
        name="iniciar_tax_refund",
    ),
    # API Analytics Anterior
    path(
        "api/reconciliacion/stats/",
        api_reconciliacion_views.ReconciliationDashboardStatsAPIView.as_view(),
        name="api_reconciliacion_stats",
    ),
    path(
        "api/reconciliacion/upload-async/",
        views_reconciliation.ReporteReconciliacionAsyncUploadAPIView.as_view(),
        name="api_reconciliacion_upload_async",
    ),
    # Staging Ledger Buffer (Propuestas de Transacción IA)
    path(
        "api/finance/propuestas/",
        ai_views.PropuestaTransaccionIAListCreateAPIView.as_view(),
        name="api_propuestas_list",
    ),
    path(
        "api/finance/propuestas/<uuid:pk>/<str:action>/",
        ai_views.ResolvePropuestaAPIView.as_view(),
        name="api_propuesta_resolve",
    ),
    ...
    path(
        "liquidaciones/", liquidaciones_views.LiquidacionListView.as_view(), name="liquidacion_list"
    ),
    path(
        "liquidaciones/nueva/",
        liquidaciones_views.LiquidacionCreateView.as_view(),
        name="liquidacion_create",
    ),
    path(
        "liquidaciones/<int:pk>/",
        liquidaciones_views.LiquidacionDetailView.as_view(),
        name="liquidacion_detail",
    ),
    # MOTOR HÍBRIDO (AUDI ENGINE v3.0)
    path(
        "api/finance/reconciliacion/upload/",
        report_upload_view.ReporteProveedorUploadAPIView.as_view(),
        name="api_finance_reconciliacion_upload",
    ),
    path(
        "api/finance/task-status/<str:task_id>/",
        task_status_view.ReconciliationTaskStatusAPIView.as_view(),
        name="api_finance_task_status",
    ),
    # MAGIC LINK CHECKOUT (B2C)
    path(
        "pay/<uuid:uuid_link>/",
        checkout_views.MagicLinkCheckoutView.as_view(),
        name="magic_link_checkout",
    ),
    # Stripe Checkout & Webhook
    path(
        "pay/<uuid:uuid_link>/stripe/",
        stripe_views.StripeCheckoutView.as_view(),
    ...
    path(
        "pricing/",
        TemplateView.as_view(template_name="billing/pricing.html"),
    ...
    path("api/billing/change-plan/", change_plan, name="change_plan"),
    path("api/billing/preview-change/", preview_plan_change, name="preview_plan_change"),
    path("api/billing/downgrade-free/", downgrade_to_free, name="downgrade_free"),
    ...
    path(
        "facturas-proveedores/revisar/",
        invoice_views.InvoiceReviewDashboardView.as_view(),
        name="invoice_review_dashboard",
    ),
    path(
        "facturas-proveedores/vincular/<int:factura_id>/",
        invoice_views.force_match_invoice_htmx,
        name="force_match_invoice",
    ),

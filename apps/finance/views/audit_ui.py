...
class ReconciliationResultsView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["discrepancias"] = (
            reporte.conciliaciones.exclude(estado="OK")
            .select_related("linea_reporte", "boleto_local", "sugerencia_asiento")
...
def approve_adjustment_htmx(request, conciliacion_id):
    ...
    if asiento:
        ...
        conciliacion.resolucion_notas = (
            f"Auditado y aprobado por {request.user.username} (IA Certified)"
        )
        conciliacion.save()
        ...
        return render(
            request,
            "finance/reconciliacion/partials/adjustment_success.html",
            {"asiento_id": asiento.id_asiento, "numero_asiento": f"AS-{asiento.id_asiento}"},
        )
    else:
        # Si falla el motor contable por descuadre
        return HttpResponse(
            '<span class="text-status-danger text-[10px] font-bold p-2 bg-status-danger-bg rounded">Falla CPA Engine</span>'
        )

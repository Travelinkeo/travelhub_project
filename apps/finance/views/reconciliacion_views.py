...
class ReconciliationDashboardHTMXView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        reportes = ReporteReconciliacion.objects.select_related("agencia").order_by(
            "-fecha_subida"
        )[:10]
        ...
        context["alertas_criticas"] = (
            ConciliacionBoleto.objects.filter(
                diferencia_total__gt=5  # Hardcoded. Podria ser configurable. Mostrar diferencias > 5 USD
            )
            .select_related("reporte", "linea_reporte", "boleto_local")
            .order_by("-diferencia_total")[:15]
        )
...
class ReporteReconciliacionCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request,
            "Reporte sincronizado de manera exitosa. Listo para ser cruzado a nivel contable.",
        )

...
def process_reconciliation_upload_htmx(request):
    ...
    try:
        ...
        if not archivo:
            return HttpResponse(
                '<p class="text-status-danger text-xs font-bold">Error: No se recibió ningún archivo.</p>'
            )
        ...
        if not task_id:
            ...
            reporte.save()
            return HttpResponse(
                '<p class="text-status-danger text-xs font-bold">Error: El motor de IA no está disponible.</p>'
            )
        ...
        return HttpResponse(
            f'<p class="text-status-danger text-xs font-bold">Error crítico: {str(e)}</p>'
        )
...
def reconciliation_task_status_htmx(request, task_id):
    ...
    if res.status == "SUCCESS":
        ...
        response["HX-Redirect"] = reverse(
            "finance:reconciliacion_detail", kwargs={"pk": reporte_id}
        )
        return response
    ...
    if res.status == "FAILURE":
        return render(
            request,
            "finance/reconciliacion/partials/task_progress.html",
            {"status": "FAILURE", "error": str(res.result), "task_id": task_id},
        )

...
class ReporteProveedorUploadAPIView(InternalAPIAuthMixin, views.APIView):
    """
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not archivo:
            return Response(
                {"error": "No se recibió ningún archivo."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            reporte = ReporteReconciliacion.objects.create(
                agencia=agencia, archivo=archivo, proveedor=proveedor, estado="PENDIENTE"
            )

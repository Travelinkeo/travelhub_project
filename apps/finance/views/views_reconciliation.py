...
class ReporteReconciliacionAsyncUploadAPIView(InternalAPIAuthMixin, views.APIView):
    """
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not hasattr(user, "perfil") or not user.perfil.agencia_id:
            raise ValidationError(
                {"error": "Su usuario no tiene una agencia asociada para realizar conciliaciones."}
            )
        ...
        if serializer.is_valid():
            ...
            reporte = serializer.save(agencia_id=agencia_id, estado="PENDIENTE")
            ...
            task = conciliar_reporte_batch_task.delay(
                reporte_id=str(reporte.id_reporte), agencia_id=agencia_id
            )

...
class PromotionService:
    ...
    def generate_instagram_story(hotel_id, agencia_id=None):
        ...
        tarifas = TarifaHabitacion.objects.filter(
            tipo_habitacion__hotel=hotel, tipo_tarifa="POR_PERSONA"
        ).order_by("tarifa_dbl")

        if tarifas.exists():
            min_price = tarifas.first().tarifa_dbl

        Agencia.objects.filter(pk=agencia_id).first() if agencia_id else Agencia.objects.filter(
            activa=True
        ).first()
        ...
        draw_text_center(
            H - 300,
            f"{hotel.destino.upper()}  |  {'⭐' * hotel.categoria}",
            font_sub,
            color="#fbbf24",
        )
        if min_price > 0:
    ...
    def generate_ai_promo_image(hotel_name, price, style="Luxurious", custom_text=None):
        try:
            ...
            images = model.generate_images(
                prompt=base_prompt, number_of_images=1, aspect_ratio="1:1"
            )

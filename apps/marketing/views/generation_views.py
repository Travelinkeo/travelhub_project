...
class GenerarFlyerView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        flyer_buffer = service.generate_flyer(
            destination=destino, price=precio, airline=aerolinea, hotel_id=hotel_id
        )
        ...
        if hotel_id:
            ...
            if hotel:
                ...
                activo.archivo.save(
                    f"flyer_{activo.id}.jpg", ContentFile(flyer_buffer.getvalue()), save=True
                )
...
class GenerarSocialMediaAdvancedView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if not agencia:
                logger.warning(
                    f"Intento de generar activo de marketing sin contexto de agencia para hotel {hotel_id}"
                )
            ...
            if isinstance(package, dict) and package.get("variants"):
                ...
                package = package if isinstance(package, dict) else package.dict()
        ...
        return render(
            request,
            "marketing/partials/social_package_result.html",
            {"package": package, "hotel": HotelTarifario.objects.get(pk=hotel_id)},
        )

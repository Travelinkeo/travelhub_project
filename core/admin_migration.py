...
class MigrationCheckAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Resultado de la Validación", {"fields": ("alert_level", "summary")}),
        ("Requisitos de Visa", {"fields": ("visa_required", "visa_type")}),
        (
            "Validez del Pasaporte",
            {"fields": ("passport_validity_ok", "passport_min_months_required")},
        ),
        ("Vacunas Requeridas", {"fields": ("vaccination_required",)}),
        (
            "Detalles Técnicos",
            {
    ...
    def ai_badge(self, obj):
        ...
        if obj.checked_by_ai:
            return format_html(
                '<span style="background: #007bff; color: white; padding: 2px 6px; border-radius: 3px; font-size: 10px;">🤖 AI</span>'
            )
        return format_html(
            '<span style="background: #6c757d; color: white; padding: 2px 6px; border-radius: 3px; font-size: 10px;">📚 DB</span>'
        )
...
def validate_migration_requirements_action(modeladmin, request, queryset):
    ...
    if validaciones_realizadas > 0:
        messages.success(
            request, f"✅ Se realizaron {validaciones_realizadas} validación(es) migratoria(s)"
        )

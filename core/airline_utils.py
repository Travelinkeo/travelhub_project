...
def validate_airline_numeric_code(ticket_number: str, airline_iata: str) -> bool:
    ...
    try:
        ...
        if aerolinea.numeric_code == prefix:
            return True
        else:
            logger.warning(f"⚠️ Discrepancia Validación Numérica: Boleto {ticket_number} (Prefijo {prefix}) != {airline_iata} (Código {aerolinea.numeric_code})")
            return False

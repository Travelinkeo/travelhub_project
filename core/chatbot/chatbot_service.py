...
"""
    ...
    def chat(self, user_message: str, conversation_history: list[dict] = None) -> dict:
        ...
        try:
            ...
            return {"success": True, "response": response_text.strip(), "fallback": False}
    ...
    def get_fallback_response(self, user_message: str) -> str:
        ...
        if any(word in message_lower for word in ["hola", "buenos", "buenas", "saludos"]):
            return (
                "¡Hola! Soy Linkeo, tu asistente virtual de TravelHub. Estoy aquí para ayudarte con información sobre "
        ...
        if any(word in message_lower for word in ["precio", "costo", "cotiza", "cuanto"]):
            return (
                "Para obtener una cotización personalizada, por favor contáctanos directamente. "
        ...
        if any(word in message_lower for word in ["reserva", "reservar", "boleto", "vuelo"]):
            return (
                "Para realizar una reserva, necesitamos algunos datos. Un agente puede ayudarte "
        ...
        if any(word in message_lower for word in ["pasaporte", "visa", "documento", "requisito"]):
            return (
                "Los requisitos de documentación varían según el destino. Te recomiendo consultar "
        ...
        if any(word in message_lower for word in ["hotel", "hospedaje", "alojamiento"]):
            return (
                "Ofrecemos una amplia selección de hoteles en diversos destinos. Para recomendaciones "
        ...
        if any(word in message_lower for word in ["paquete", "tour", "excursion"]):
            return (
                "Tenemos paquetes turísticos para diversos destinos. Para ver opciones disponibles "
        ...
        # Respuesta genérica
        return (
            "Gracias por tu consulta. Para brindarte la mejor atención, te recomiendo contactar "

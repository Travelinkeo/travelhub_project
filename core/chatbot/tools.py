...
def agregar_servicio_adicional(pnr: str, id_servicio: int) -> dict:
    ...
    logger.info(f"SIMULACIÓN: Agregando servicio {id_servicio} a la reserva {pnr}.")
    return {
        "status": "success",
...
def buscar_paquetes_turisticos(destino: str, presupuesto: float, mes: str) -> dict:
    ...
    logger.info(
        f"SIMULACIÓN: Buscando paquetes para {destino} en {mes} con presupuesto de ${presupuesto}."
    )
    return {
        "status": "success",
        "data": [
            {
                "nombre": "París Mágico",
                "precio": 1200,
                "descripcion": "7 días en el corazón de París, incluye hotel y tour a la Torre Eiffel.",
            },
            {
                "nombre": "Aventura en los Alpes",
                "precio": 1450,
                "descripcion": "8 días de senderismo y vistas espectaculares en los Alpes franceses.",
            },
        ],
    }

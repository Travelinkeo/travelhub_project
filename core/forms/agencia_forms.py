...
class AgenciaSettingsForm(forms.ModelForm):
    ...
    color_primario = forms.CharField(
        widget=forms.TextInput(attrs={"type": "color"}), required=False
    )
    ...
    def save(self, commit=True):
        ...
        if commit:
            ...
            # Guardar Branding - usar el FK directo si ya existe, o get_or_create por la FK inversa
            if agencia.branding:
                branding = agencia.branding
            else:
                branding, created = AgenciaBranding.objects.get_or_create(agencia=agencia)
                if created or agencia.branding_id != branding.pk:
                    # Vincular el nuevo branding al campo FK directo de Agencia
                    agencia.branding = branding
                    agencia.save(update_fields=["branding"])
            ...
            for field in branding_fields:
                if field in self.cleaned_data:
                    ...
                    # Para ImageFields: None significa "no envío archivo" (no borrar el existente)
                    # False puede significar "borrar archivo" en Django - lo ignoramos
                    if field in ["logo", "logo_light", "logo_dark"]:
                        if val is None or val is False:
                            # No se subió ninguna imagen nueva, mantener la existente
                            continue
            ...
            if agencia.configuracion:
                ...
                config, created = AgenciaConfiguracion.objects.get_or_create(agencia=agencia)
                if created or agencia.configuracion_id != config.pk:

...
class PasajeroForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "fecha_vencimiento_documento": forms.DateInput(
                attrs={
                    "type": "date",
                    "class": "bg-gray-800/50 border border-gray-700 text-white rounded-xl px-4 py-2 w-full",
                }
    ...
    def clean(self):
        ...
        if cleaned_data.get("pref_comida_veg"):
            preferencias["comida"].append("VGML")
        if cleaned_data.get("pref_comida_kosher"):
            preferencias["comida"].append("KSML")
        if cleaned_data.get("pref_comida_sin_gluten"):
            preferencias["comida"].append("GFML")

        if cleaned_data.get("pref_asistencia_silla"):
            preferencias["asistencia"].append("WCHR")

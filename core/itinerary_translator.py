...
class ItineraryTranslator:
    ...
    def translate_itinerary(self, itinerary: str, gds_system: str = "SABRE") -> dict[str, Any]:
        ...
        result = {"html": "", "structured_data": [], "error": None}
        ...
        try:
            ...
            if "vuelos" in data and data["vuelos"]:
                for vuelo in data["vuelos"]:
                    ...
                    logo_html = (
                        f'<img src="{logo_url}" alt="{iata_code}" class="w-16 h-16 mr-4 object-contain">'
                        if logo_url
                        else ""
                    )
            ...
            # Caso 2: KIU (ItinerarioFinalLimpio string)
                ...
                result["html"] = (
                    '<div class="warning p-4 bg-yellow-50 text-yellow-700 rounded-md">Se detectaron datos pero no se pudo estructurar el itinerario.</div>'
                )
                return result
            ...
            result["html"] = (
                '<div class="error p-4 bg-red-50 text-red-700 rounded-md">Error proceso interno al traducir.</div>'
            )
            result["error"] = str(e)
            return result
...
class TicketCalculator:
    ...
    def calculate_ticket_price(
        tarifa: Decimal, fee_consolidador: Decimal, fee_interno: Decimal, porcentaje: Decimal
    ) -> dict[str, Any]:
        """

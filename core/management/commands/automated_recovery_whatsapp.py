...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.NOTICE(f"Iniciando proceso de recuperación financiera (Dry Run: {dry_run})"))
        ...
        if dry_run:
            ...
            for ag in list(overdue) + list(expired_trials):
                self.stdout.write(f"  - Se notificaría a: {ag.nombre} (WA: {ag.whatsapp or 'No configurado'})")
        else:
            results = CollectionAIService.process_overdue_accounts()
            
            for res in results:
                status = self.style.SUCCESS("EXITO") if res.get('success') else self.style.ERROR("FALLO")
                self.stdout.write(f"Agencia: {res['agencia']} | Resultado: {status} | Error: {res.get('error', 'Ninguno')}")

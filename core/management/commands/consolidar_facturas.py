...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        total = Factura.objects.all().count()
        ...
        for factura in Factura.objects.all().iterator(chunk_size=1000):
            try:
                with transaction.atomic():
                    # Verificar si ya existe
                    if FacturaConsolidada.objects.filter(
                    ...
                    emisor_rif = (
                        agencia.rif if agencia and hasattr(agencia, "rif") else "J-00000000-0"
                    )
                    emisor_razon_social = agencia.nombre if agencia else "Agencia de Viajes"
                    emisor_direccion = (
                        agencia.direccion
                        if agencia and hasattr(agencia, "direccion")
                        else "Dirección fiscal"
                    )
                    ...
                    if not dry_run:
                        ...
                        self.stdout.write(
                            self.style.SUCCESS(f"OK [DRY-RUN] Migraria: {factura.numero_factura}")
                        )
                ...
                errores += 1
                self.stdout.write(
                    self.style.ERROR(f"ERROR migrando {factura.numero_factura}: {str(e)}")
                )

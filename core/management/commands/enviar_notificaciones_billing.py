...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        trials_7_dias = Agencia.objects.filter(plan="FREE", fecha_fin_trial=en_7_dias, activa=True)

        for agencia in trials_7_dias:
            try:
                enviar_email_trial_expirando(agencia, 7)
                self.stdout.write(self.style.SUCCESS(f"Email enviado a {agencia.nombre} (7 días)"))
            except Exception as e:
                self.stdout.write(self.style.ERROR(f"Error: {e}"))

        # 2. Trials que expiran en 3 días
        en_3_dias = hoy + timedelta(days=3)
        trials_3_dias = Agencia.objects.filter(plan="FREE", fecha_fin_trial=en_3_dias, activa=True)

        for agencia in trials_3_dias:
            try:
                enviar_email_trial_expirando(agencia, 3)
                self.stdout.write(self.style.SUCCESS(f"Email enviado a {agencia.nombre} (3 días)"))
            except Exception as e:
                self.stdout.write(self.style.ERROR(f"Error: {e}"))

        # 3. Trials expirados hoy
        trials_expirados = Agencia.objects.filter(plan="FREE", fecha_fin_trial=hoy, activa=True)

        for agencia in trials_expirados:
            try:
                enviar_email_trial_expirado(agencia)
                self.stdout.write(
                    self.style.SUCCESS(f"Email de expiración enviado a {agencia.nombre}")
                )
            except Exception as e:
                self.stdout.write(self.style.ERROR(f"Error: {e}"))
        ...
        for agencia in agencias_limite.iterator(chunk_size=100):
            porcentaje_uso = (

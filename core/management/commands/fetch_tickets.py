    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if not all([GMAIL_USER, GMAIL_APP_PASSWORD]):
                self.stdout.write(
                    self.style.ERROR(
                        "Error Crítico: Las variables de entorno GMAIL_USER y GMAIL_APP_PASSWORD no están configuradas en settings.py."
                    )
                )
                return
            ...
            search_criteria = (
                f'(FROM "{GMAIL_FROM_KIU}" SUBJECT "E-TICKET ITINERARY RECEIPT" UNSEEN)'
            )
            status, messages = mail.search(None, search_criteria)

            if status != "OK" or not messages[0]:
                self.stdout.write(
                    self.style.SUCCESS("No se encontraron nuevos correos para procesar.")
                )
                mail.logout()
                return
            ...
            self.stdout.write(
                self.style.WARNING(f"Se encontraron {len(email_ids)} correo(s) nuevo(s).")
            )
            ...
            for email_id in email_ids:
                try:
                    ...
                    if status != "OK":
                        self.stdout.write(
                            self.style.ERROR(
                                f"No se pudo obtener el contenido del email ID {email_id.decode()}."
                            )
                        )
                        continue
                    ...
                    sanitized_subject = "".join(
                        c for c in subject if c.isalnum() or c in (" ", "-")
                    ).rstrip()
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"Boleto guardado con el ID {boleto.id_boleto_importado} y el archivo {filename}."
                        )
                    )
                    self.stdout.write(
                        "El procesamiento automático (parseo) se ha iniciado en segundo plano."
                    )
                    ...
                    self.stdout.write(
                        self.style.ERROR(f"Error procesando el email ID {email_id.decode()}: {e}")
                    )
                    import traceback
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Proceso completado. Se procesaron {processed_count} de {len(email_ids)} correos."
                )
            )

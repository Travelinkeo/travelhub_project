...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options["mes"] and options["anio"]:
            ...
            self.stdout.write(
                self.style.ERROR("Debe especificar --mes y --anio, o --fecha-inicio y --fecha-fin")
            )
            return
        ...
        if options["formato"] == "csv":
            ...
            self.stdout.write(
                f"  - Ventas Terceros: {libro_ventas['resumen']['facturas_terceros']}"
            )

            self.stdout.write("\nVENTAS PROPIAS:")
            self.stdout.write(
                f"  Base Gravada:    ${libro_ventas['totales']['propias']['base_gravada']:>12.2f}"
            )
            self.stdout.write(
                f"  Base Exenta:     ${libro_ventas['totales']['propias']['base_exenta']:>12.2f}"
            )
            self.stdout.write(
                f"  Base Exportación:${libro_ventas['totales']['propias']['base_exportacion']:>12.2f}"
            )
            self.stdout.write(
                f"  IVA 16%:         ${libro_ventas['totales']['propias']['iva_16']:>12.2f}"
            )
            self.stdout.write(
                f"  IVA Adicional:   ${libro_ventas['totales']['propias']['iva_adicional']:>12.2f}"
            )
            self.stdout.write(
                f"  IGTF:            ${libro_ventas['totales']['propias']['igtf']:>12.2f}"
            )
            self.stdout.write(
                f"  Total:           ${libro_ventas['totales']['propias']['total']:>12.2f}"
            )

            if libro_ventas["ventas_terceros"]:
                self.stdout.write("\nVENTAS TERCEROS:")
                self.stdout.write(
                    f"  Base Exenta:     ${libro_ventas['totales']['terceros']['base_exenta']:>12.2f}"
                )
                self.stdout.write(
                    f"  Total:           ${libro_ventas['totales']['terceros']['total']:>12.2f}"
                )

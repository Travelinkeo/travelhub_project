...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--only",
            nargs="*",
            help="Limitar a uno o varios catálogos: paises monedas ciudades aerolineas proveedores clientes productos_servicios",
        )
        parser.add_argument(
            "--format",
            choices=["json", "csv", "auto"],
            default="auto",
            help="Formato fuente si se usan archivos CSV alternativos",
        )
        parser.add_argument(
            "--dir", default=".", help="Directorio base donde buscar la carpeta fixtures o CSVs"
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
        ...
        parser.add_argument(
            "--upsert",
            action="store_true",
            help="Actualiza registros existentes basados en claves únicas",
        )
    ...
    def handle(self, *args, **options):
        ...
        for cat in catalogs:
            ...
            if not file_path.exists():
                self.stdout.write(
                    self.style.WARNING(f"[SKIP] {cat}: archivo no encontrado {file_path}")
                )
                continue
            if fmt == "auto":
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"[{cat}] creados={created} actualizados={updated} omitidos={skipped}"
                )
            )
        ...
        try:
            if catalog == "paises":
                return self._upsert_model(
                    Pais,
                    {"codigo_iso_2": row_upper_keys.get("codigo_iso_2")},
                    {
            ...
            if catalog == "monedas":
                return self._upsert_model(
                    Moneda,
                    {"codigo_iso": row_upper_keys.get("codigo_iso")},
                    {
                        "nombre": row_upper_keys.get("nombre"),
            ...
            if catalog == "ciudades":
                ...
                pais_codigo = row_upper_keys.get("pais_codigo_iso_2") or row_upper_keys.get(
                    "pais_codigo_iso_3"
                )
                pais_obj = None
                if pais_codigo:
                    ...
                    pais_obj = (
                        Pais.objects.filter(codigo_iso_2__iexact=pais_codigo).first()
                        or Pais.objects.filter(codigo_iso_3__iexact=pais_codigo).first()
                    )
                if not pais_obj:
                    ...
                    if simulate:
                        return 0, 0, 1
                    raise CommandError(
                        f"País no encontrado para ciudad '{row_upper_keys.get('nombre')}' con código '{pais_codigo}'"
                    )
                lookup = {
                    "nombre": row_upper_keys.get("nombre"),
                    "pais": pais_obj,
            ...
            if catalog == "proveedores":
                return self._upsert_model(
                    ...
                    {"nombre": row_upper_keys.get("nombre")},
                    {
                        "tipo_proveedor": row_upper_keys.get("tipo_proveedor")
                        or Proveedor.TipoProveedorChoices.OTRO,
                        "nivel_proveedor": row_upper_keys.get("nivel_proveedor")
                        or Proveedor.NivelProveedorChoices.DIRECTO,
            ...
            if catalog == "clientes":
                ...
                lookup = (
                    {"email": row_upper_keys.get("email")}
                    if row_upper_keys.get("email")
                    else {
            ...
            if catalog == "aerolineas":
                return self._upsert_model(
                    Aerolinea,
                    {"codigo_iata": row_upper_keys.get("codigo_iata")},
                    {
                        "nombre": row_upper_keys.get("nombre"),
            ...
            if catalog == "productos_servicios":
                return self._upsert_model(
                    ...
                    {"nombre": row_upper_keys.get("nombre")},
                    {
                        "tipo_producto": row_upper_keys.get("tipo_producto")
                        or ProductoServicio.TipoProductoChoices.OTRO,
                    },
                    upsert,
                    simulate,
                )

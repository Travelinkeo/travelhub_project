...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
            ...
            try:
                ...
                if agencia.logo:
                    try:
                        ...
                        self.stdout.write(
                            self.style.SUCCESS(
                                f"Logo migrado para agencia: {agencia.nombre} a partir de logo ImageField"
                            )
                        )
                        migradas += 1
                        logo_generado = True
                        ...
                        self.stdout.write(
                            self.style.WARNING(
                                f"No se pudo leer logo ImageField de {agencia.nombre}: {e}"
                            )
                        )
                ...
                if not logo_generado and agencia.logo_base64 and not agencia.logo_pdf_base64:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"Logo migrado para agencia: {agencia.nombre} a partir de logo_base64"
                        )
                    )
                    migradas += 1
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Migración completada. Exitosas: {migradas}. Errores: {errores}.")
        )

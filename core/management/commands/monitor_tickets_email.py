...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--all", action="store_true", help="Procesar todos los correos (incluso leidos)"
        )
        parser.add_argument("--force", action="store_true", help="Reprocesar boletos existentes")
        parser.add_argument(
            "--agencia", type=str, help="Nombre de la agencia a procesar (opcional)"
        )
    ...
    def handle(self, *args, **options):
        ...
        if options["agencia"]:
            ...
            if not target_agencia:
                self.stdout.write(
                    self.style.ERROR(f"No se encontró agencia con nombre '{options['agencia']}'")
                )
                return
            ...
            self.stdout.write(
                self.style.WARNING("Hay múltiples agencias. Usa --agencia para especificar una.")
            )
            for ag in agencias:

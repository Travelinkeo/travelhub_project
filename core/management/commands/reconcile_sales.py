    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if not ocr_text.strip():
                self.stderr.write(
                    self.style.ERROR(
                        "No se pudo extraer texto del PDF. El archivo podría estar vacío o ser una imagen sin OCR."
                    )
                )
                return
            ...
            if not parsed_sales:
                self.stderr.write(
                    self.style.ERROR(
                        "La IA no pudo parsear el reporte. Verifique el log para más detalles."
                    )
                )
                return

            self.stdout.write(
                self.style.SUCCESS(f"{len(parsed_sales)} registros de ventas parseados con éxito.")
            )
            ...
            for sale in parsed_sales:
                ...
                if not ticket_number or report_total_str is None:
                    self.stdout.write(
                        self.style.WARNING(
                            f"Registro ignorado en el reporte por falta de datos: {sale}"
                        )
                    )
                    continue
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f"Valor inválido para 'monto_a_pagar' en el reporte para boleto {ticket_number}: {report_total_str}"
                        )
                    )
                    continue
                ...
                try:
                    ...
                    self.stderr.write(
                        self.style.ERROR(f"Error inesperado procesando boleto {ticket_number}: {e}")
                    )
            ...
            self.stdout.write(
                self.style.SUCCESS(f"- Boletos encontrados y conciliados: {found_count}")
            )
            self.stdout.write(
                self.style.ERROR(f"- Boletos no encontrados en la BD: {not_found_count}")
            )
            self.stdout.write(
                self.style.WARNING(f"- Boletos con discrepancias de monto: {discrepancy_count}")
            )
            self.stdout.write("-" * 50)

        except FileNotFoundError:
            self.stderr.write(
                self.style.ERROR(f"El archivo no fue encontrado en la ruta: {report_path}")
            )
        except Exception as e:
            self.stderr.write(
                self.style.ERROR(f"Ocurrió un error inesperado durante la conciliación: {e}")
            )

...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not user_desktop_source.exists():
            self.stdout.write(
                self.style.ERROR(
                    f"❌ Error: No se encontró la carpeta en el escritorio: {user_desktop_source}"
                )
            )
            self.stdout.write(
                self.style.WARNING(
                    'Por favor, asegúrate de que la carpeta "Referencias GDS" esté en tu escritorio.'
                )
            )
            return
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"🚀 Iniciando sincronización de conocimiento desde {user_desktop_source}..."
            )
        )
        ...
        gds_mapping = {"AMADEUS": "AMADEUS", "SABRE": "SABRE", "KIU": "KIU"}
                ...
                for file_entry in os.scandir(entry.path):
                    if file_entry.is_file():
                        ...
                        if ext in [".md", ".txt"]:
                            ...
                            clean_filename = (
                                filename if ext == ".md" else filename.replace(".txt", ".md")
                            )
                            ...
                            try:
                                ...
                                self.stdout.write(
                                    self.style.ERROR(
                                        f"  [ERR] No se pudo copiar {filename}: {str(e)}"
                                    )
                                )
                                errors_count += 1

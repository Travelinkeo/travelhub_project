...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if ruta.startswith("http"):
            ...
            try:
                response = requests.get(ruta, timeout=30)  # noqa: S113
                if response.status_code == 200:
                    archivo_contenido = response.content
                    self.stdout.write(
                        self.style.SUCCESS("   - ✅ Archivo descargado a memoria ram.")
                    )
                else:
                    self.stdout.write(
                        self.style.ERROR(f"   - ❌ Error descargando: {response.status_code}")
                    )
                    return
        ...
        boleto = BoletoImportado.objects.create(
            archivo_boleto=archivo_simulado, agencia=agencia, estado_parseo="PEN"
        )
        ...
        try:
            ...
            if venta:
                self.stdout.write(
                    self.style.SUCCESS(
                        f"🎉 ¡ÉXITO! Venta: {venta.total_venta} {venta.moneda_operacion}"
                    )
                )
            else:

...
class Command(BaseCommand):  # noqa: F811
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.WARNING(f"Iniciando pruebas de WebReceiptParser en: {base_path}")
        )
        ...
        for f_name in files:
            ...
            if f_name.endswith(".eml"):
                with open(path, "rb") as f:
                    ...
                    if body:
                        ...
                        self.stdout.write(
                            self.style.ERROR("  [!] No se encontró parte HTML en el EML")
                        )
                        continue
            ...
            if result.get("error"):
                ...
                self.stdout.write(
                    self.style.WARNING(f"  [!] Multi-Pax Mode: {result.get('is_multi_pax')}")
                )
                ...
                for _i, t in enumerate(tickets):
                    ...
                    self.stdout.write(
                        f"        Monto: {t.get('TOTAL_MONEDA')} {t.get('TOTAL_IMPORTE')}"
                    )

                    for v in t.get("vuelos", []):
                        self.stdout.write(
                            f"           - {v.get('origen')} > {v.get('destino')} ({v.get('fecha')} {v.get('hora_salida')})"
                        )
                    ...
                    if t.get("CODIGO_IDENTIFICACION") == "PENDIENTE":
                        ...
                        try:
                            ...
                            if manual_id:
                                ...
                                self.stdout.write(
                                    self.style.SUCCESS(
                                        f"      [OK] ID Actualizado a: {t['CODIGO_IDENTIFICACION']}"
                                    )
                                )

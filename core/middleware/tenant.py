...
from .rls import get_rls_bypass_flag, rls_session_context
...
class ThreadLocalContextMiddleware:
    ...
    def __call__(self, request):
        ...
        try:
            ...
            # 5. Establecer contexto RLS en la base de datos usando el context manager
            tenant_id = str(agency.id) if agency else "0"
            is_impersonating_val = (
                user and user.is_superuser and request.session.get("impersonated_agencia_id")
            )
            is_admin_path = str(request.path).startswith("/admin/")
            bypass = get_rls_bypass_flag(user, request.path, is_impersonating_val)

            with rls_session_context(
                tenant_id=tenant_id,
                bypass_rls=bypass,
                is_admin_path=is_admin_path,
            ):
                pass  # RLS configurado, ahora procesamos la request

...
class TramoVueloSchema(BaseModel):
    ...
    numero_vuelo: str | None = Field(default=None, description="Número de vuelo INCLUYENDO EL CÓDIGO DE AEROLÍNEA (ej: TK0224, CM062)")
    origen: str = Field(description="Nombre completo de la ciudad de origen (ej. BOGOTA, CARACAS)")
    codigo_iata_origen: str | None = Field(default=None, description="Código IATA de 3 letras de la ciudad de origen (ej. BOG, CCS)")
    ...
    codigo_iata_destino: str | None = Field(default=None, description="Código IATA de 3 letras de la ciudad de destino (ej. BOG, MAD)")
    hora_llegada: str = Field(description="Hora de llegada en formato 24h HH:MM (ej: 15:10). NUNCA usar AM/PM.")
    fecha_llegada: str | None = Field(default=None, description="Fecha de llegada en formato GDS DDMMMAA en mayúsculas (ej: 29MAR26).")
    cabina: str | None = Field(default=None, description="Clase de cabina (Económica, Ejecutiva, etc.)")
    clase: str | None = Field(default=None, description="Clase tarifaria (clase de reserva, ej: Y, M, L)")
    localizador_aerolinea: str | None = Field(default=None, description="Localizador específico de la aerolínea si difiere del principal")
...
class BoletoAereoSchema(BaseModel):
    ...
    codigo_identificacion: str | None = Field(default=None, description="FOID, DNI, Cédula o Pasaporte del pasajero (sin prefijos)")
    solo_nombre_pasajero: str = Field(description="Únicamente el primer nombre del pasajero limpio")
    numero_boleto: str | None = Field(default=None, description="Número de boleto de 13 dígitos. Obligatorio si existe. Null si es Low-Cost")
    fecha_emision: str | None = Field(default=None, description="Fecha de emisión en formato DDMMMAA (ej: 14MAR26)")
    agente_emisor: str | None = Field(default=None, description="Código IATA o Identificador de la oficina/agente emisor")
    numero_iata: str | None = Field(default=None, description="Número IATA de la agencia (8 dígitos exactos)")
    codigo_reserva: str = Field(description="Localizador principal de la reserva (PNR) exactamente 6 caracteres alfanuméricos")
    codigo_reserva_aerolinea: str | None = Field(default=None, description="Localizador específico de la aerolínea (si es diferente al principal)")
    nombre_aerolinea: str | None = Field(default=None, description="Nombre de la aerolínea principal o validadora")
    direccion_aerolinea: str | None = Field(default=None, description="Dirección física de la aerolínea (si está presente)")
    ...
    tarifa: float = Field(default=0.0, description="Monto numérico de la tarifa base (solo dígitos)")
    impuestos: float = Field(default=0.0, description="Monto numérico total de impuestos (solo dígitos)")
    total: float = Field(default=0.0, description="Monto total pagado. DEBE ser igual a tarifa + impuestos.")
    moneda: str = Field(default="USD", description="Código de moneda (ej: USD, VES, EUR)")
    es_remision: bool = Field(default=False, description="Indica si es una re-emisión (detectable por 'A' en total o Tarifa > Total)")
    source_system: str = Field(default="UNKNOWN", description="Sistema de origen detectado (KIU, SABRE, AMADEUS, WINGO, COPA_SPRK, etc.)")

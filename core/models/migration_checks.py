...
class MigrationCheck(models.Model):
    ...
    fecha_viaje = models.DateField(null=True, blank=True, help_text="Fecha del primer vuelo")
    ...
    visa_required = models.BooleanField(
        null=True, blank=True, help_text="¿Requiere visa para el destino?"
    )
    ...
    passport_validity_ok = models.BooleanField(
        null=True, blank=True, help_text="¿El pasaporte tiene validez suficiente?"
    )
    ...
    vaccination_required = models.JSONField(
        default=list, help_text="Lista de vacunas requeridas (ej: ['Yellow Fever', 'COVID-19'])"
    )

    # Detalles y metadata
    summary = models.TextField(null=True, blank=True, help_text="Resumen legible de la validación")
    full_report = models.JSONField(default=dict, help_text="Reporte completo en formato JSON")

    # Auditoría
    checked_at = models.DateTimeField(auto_now_add=True, help_text="Fecha y hora de la validación")

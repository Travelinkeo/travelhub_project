...
class CopaParser(BaseTicketParser):
    ...
    def _extract_passenger(self, text: str) -> Dict[str, str]:
        ...
        # Buscar: LEDY VANESSA PEREZ (ADT)
        match = re.search(r'([A-ZÁÉÍÓÚÑ\s]+?)\s*\(ADT\)', text)
        if match:
            return {
    ...
    def _extract_flights(self, text: str) -> List[Dict[str, Any]]:
        ...
        # Estrategia: Buscar "Copa Airlines" y el número de vuelo en la celda adyacente
        # HTML: Copa Airlines</td><td ...>398</td>
        # Pattern: Copa Airlines\s*</td>\s*<td[^>]*>\s*(\d{1,4})\s*</td>
        
        # Primero encontramos los números de vuelo para iterar sobre ellos
        flight_candidates = re.finditer(r'Copa Airlines\s*</td>\s*<td[^>]*>\s*(\d{1,4})\s*</td>', text, re.IGNORECASE | re.DOTALL)
        
        flight_numbers = []
        for match in flight_candidates:
            num = match.group(1)
            if num not in flight_numbers:
                flight_numbers.append(num)
        
        if not flights:
            logger.info("Intentando estrategia 3: Extracción basada en texto plano (SPRK Export)")
            # 173 Miami International Apt, US MI. 15ABR. 06:00 AM Panama City Tocumen International, PA MI. 15ABR. 08:06 AM L M
            # Regex: NroVuelo + Origen + Fecha + HoraDep + Destino + Fecha + HoraArr
            # Optimizada para no capturar el "Terminal:J" si estorba
            text_clean = text.replace('\n', ' ').replace('\r', ' ')
            text_clean = re.sub(r'\s+', ' ', text_clean)
            
            # Buscamos números de vuelo precedidos por Copa Airlines
            candidates = re.finditer(r'Copa Airlines\s+(\d{1,4})', text_clean, re.IGNORECASE)
            for m in candidates:
                f_num = m.group(1)
                # Buscamos el bloque de este vuelo
                # 173 Miami... 06:00 AM Panama... 08:06 AM
                # Nota: Los aeropuertos suelen terminar en ", XX" (código país)
                move_pos = m.end()
                chunk = text_clean[move_pos:move_pos+400]
                
                # Regex para el segmento
                # (Origen), (Pais) (DiaSem). (Fecha). (Hora) (Destino), (Pais) (DiaSem). (Fecha). (Hora)
                seg_regex = re.compile(
                    r'(.*?),\s*[A-Z]{2}.*?(\d{2}:\d{2}\s*[AP]M)\s+(.*?),\s*[A-Z]{2}.*?(\d{2}:\d{2}\s*[AP]M)',
                    re.IGNORECASE
                )
                seg_match = seg_regex.search(chunk)
                if seg_match:
                    # Extraer fecha (ej: 15ABR) buscando antes de la hora
                    date_match = re.search(r'(\d{1,2}[A-Z]{3})', chunk[:seg_match.start(2)], re.IGNORECASE)
                    ...
                    flights.append({
                        ...
                        'hora_salida': seg_match.group(2),
                        'destino': seg_match.group(3).strip(),
                        'hora_llegada': seg_match.group(4),
                        'cabina': 'Económica',
                        'clase': 'L'
                    })

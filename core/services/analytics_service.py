...
class AnalyticsService:
    ...
    def get_aerolineas_disponibles(agencia):
        ...
        """
        return BoletoImportado.objects.filter(
            agencia=agencia
        ).exclude(
            aerolinea_emisora__isnull=True
        ).exclude(
            aerolinea_emisora=''
        ).values_list('aerolinea_emisora', flat=True).distinct().order_by('aerolinea_emisora')
    ...
    def get_reporte_comisiones_boletos(agencia, fecha_inicio=None, fecha_fin=None, aerolinea=None):
        ...
        for b in qs.order_by('-fecha_emision_boleto')[:100]:
            ...
            if b.numero_boleto and len(b.numero_boleto) >= 3:
                ...
                prefijos = {'134': 'AV', '230': 'CM', '045': 'LA', '001': 'AA', '057': 'AF', '074': 'KL', '239': 'AT'}
            ...
            if not aero_codigo and b.datos_parseados:
                ...
                try:
                    ...
                    if segmentos and 'vuelo' in segmentos[0]:
                        ...
                        if match: aero_codigo = match.group(1)
                except: pass
    ...
    def exportar_reporte_boletos_excel(agencia, fecha_inicio=None, fecha_fin=None, aerolinea=None):
        ...
        """
        reporte = AnalyticsService.get_reporte_comisiones_boletos(agencia, fecha_inicio, fecha_fin, aerolinea)
        
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            # 1. Resumen General
            resumen_data = {
                'Métrica': ['Total Boletos', 'Total Ventas', 'Total Neto', 'Total Comisiones', 'Total Pendiente'],
                'Valor': [
            ...
            if reporte['por_aerolinea']:
                ...
                df_aerolinea.columns = ['Aerolínea', 'Cant. Boletos', 'Total Ventas', 'Total Comisiones']
            ...
            qs = BoletoImportado.objects.filter(agencia=agencia, estado_parseo='COM')
            if fecha_inicio: qs = qs.filter(fecha_emision_boleto__gte=fecha_inicio)
            if fecha_fin: qs = qs.filter(fecha_emision_boleto__lte=fecha_fin)
            if aerolinea: qs = qs.filter(aerolinea_emisora=aerolinea)
            ...
            if detalles_full:
                pd.DataFrame(detalles_full).to_excel(writer, sheet_name='Detalle Boletos', index=False)
    ...
    def get_stats_graficas_boletos(agencia, fecha_inicio=None, fecha_fin=None):
        ...
        top_aero = qs.values('aerolinea_emisora').annotate(
            total=Sum('total_boleto')
        ).order_by('-total')[:5]
        ...
        # Si el rango es pequeño (ej. 1 mes), tal vez sea mejor por día, 
        # pero por ahora lo dejamos por mes.
        evolucion = qs.annotate(
            month=TruncMonth('fecha_emision_boleto')
        ).values('month').annotate(
            comisiones=Sum('comision_agencia'),
            ventas=Sum('total_boleto')
        ).order_by('month')
        
        labels_evolucion = [item['month'].strftime('%b %Y') for item in evolucion]
        data_comisiones = [float(item['comisiones'] or 0) for item in evolucion]
        data_ventas = [float(item['ventas'] or 0) for item in evolucion]

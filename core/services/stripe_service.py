...
class StripeService:
    ...
    def _provision_new_agency(session, auth_method='password'):
        ...
        if not admin_email or not agency_name or not subdomain:
            logger.error('Missing required metadata in Stripe checkout session %s', session.get('id'))
            return
        ...
        try:
            from core.services.magic_link_service import create_magic_link, send_magic_link_email
        ...
        cache.set(idempotency_key, True, 86400)
        logger.info('Agency provisioned: %s (%s) plan=%s auth=%s', agency_name, subdomain, plan, auth_method)

...
from core.models.ai_schemas import BoletoAereoSchema, _to_float
...
def map_legacy_to_schema(datos):
        ...
        vuelos.append(
            {
                ...
                "origen": origen,
                "codigo_iata_origen": v.get("codigo_iata_origen")
                or v.get("codigo_origen")
                or (origen if len(str(origen)) == 3 else None),
                "fecha_salida": fecha_salida,
                "hora_salida": v.get("hora_salida") or v.get("TIME") or "00:00",
                "destino": destino,
                "codigo_iata_destino": v.get("codigo_iata_destino")
                or v.get("codigo_destino")
    ...
    mapped = {
        "nombre_pasajero": datos.get("NOMBRE_DEL_PASAJERO")
        or datos.get("passenger_name")
        or "No encontrado",
        "codigo_identificacion": datos.get("CODIGO IDENTIFICACION")
        or datos.get("CODIGO_IDENTIFICACION")
        or datos.get("FOID"),
        "solo_nombre_pasajero": datos.get("SOLO NOMBRE PASAJERO")
        or datos.get("SOLO_NOMBRE_PASAJERO")
        or datos.get("solo_nombre_pasajero")
        or "No encontrado",
        "numero_boleto": datos.get("NUMERO DE BOLETO")
        or datos.get("NUMERO_DE_BOLETO")
        or datos.get("ticket_number"),
        "fecha_emision": datos.get("FECHA DE EMISION")
        or datos.get("FECHA_DE_EMISION")
        ...
        "codigo_reserva": datos.get("SOLO CODIGO RESERVA")
        or datos.get("SOLO_CODIGO_RESERVA")
        or datos.get("CODIGO_RESERVA")
        or datos.get("pnr")
        or "N/A",
        "codigo_reserva_aerolinea": datos.get("CODIGO_RESERVA_AEROLINEA")
        or datos.get("codigo_reserva_aerolinea"),
        "nombre_aerolinea": datos.get("NOMBRE AEROLINEA")
        or datos.get("NOMBRE_AEROLINEA")
        or datos.get("airline_name")
        or "No encontrado",
        ...
        "confidence_score": _to_float(
            datos.get("confidence_score") or datos.get("CONFIDENCE_SCORE", 1.0)
        ),
...
def test_parse_ticket_snapshot(file_path):
    ...
    try:
        if isinstance(datos, list):
            ...
            sanitized_datos = DataNormalizationService.sanitize_for_json(
                [obj.model_dump() for obj in valid_objects]
            )
            ...
            sanitized_datos = DataNormalizationService.sanitize_for_json(
                [obj.model_dump() for obj in valid_tickets]
            )
        else:

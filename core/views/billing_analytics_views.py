...
def get_mrr(request):
    ...
    for item in agencias_activas:
        ...
        mrr = price * count
        mrr_total += mrr
        mrr_por_plan[plan] = {"agencias": count, "precio": price, "mrr": mrr}
...
def get_churn_rate(request):
    ...
    canceladas = Agencia.objects.filter(activa=False, fecha_actualizacion__gte=hace_30_dias).count()

    # Total de agencias activas hace 30 días
    total_hace_30_dias = Agencia.objects.filter(fecha_creacion__lt=hace_30_dias).count()
    ...
    for plan in ["FREE", "BASIC", "PRO", "ENTERPRISE"]:
        ...
        if agencias_plan.exists():
            metrics_por_plan[plan] = {
                ...
                "promedio_ventas": agencias_plan.aggregate(Avg("ventas_mes_actual"))[
                    "ventas_mes_actual__avg"
                ]
                or 0,
                "promedio_usuarios": agencias_plan.annotate(
                    num_usuarios=Count("usuarios", filter=Q(usuarios__activo=True))
                ).aggregate(Avg("num_usuarios"))["num_usuarios__avg"]
                or 0,
                "uso_ventas_porcentaje": round(
                    (
                        agencias_plan.aggregate(Avg("ventas_mes_actual"))["ventas_mes_actual__avg"]
                        or 0
                    )
                    / (agencias_plan.first().limite_ventas_mes or 1)
                    * 100,
                    1,
                )
                if agencias_plan.first()
                else 0,
            }
...
def get_conversion_funnel(request):
    ...
    agencias_trial_activo = Agencia.objects.filter(
        plan="FREE", fecha_fin_trial__gte=timezone.now().date()
    ).count()
    agencias_trial_expirado = Agencia.objects.filter(
        plan="FREE", fecha_fin_trial__lt=timezone.now().date()
    ).count()

...
def change_plan(request):
    ...
    try:
        ...
        if not agencia.stripe_subscription_id:
            return Response(
                {"error": "No tienes suscripción activa", "action": "create_checkout"}, status=400
            )
...
def downgrade_to_free(request):
    ...
    try:
        ...
        stripe.Subscription.modify(agencia.stripe_subscription_id, cancel_at_period_end=True)

...
from core.models import BoletoImportado, Venta, ItemVenta, SolicitudAnulacion
...
def boletos_sin_venta(request):
    ...
    if not request.user.is_superuser and hasattr(request.user, 'agencias'):
        ua = request.user.agencias.filter(activo=True).first()
        if ua:
            boletos = boletos.filter(agencia=ua.agencia)
        else:
            return Response([])
            
    boletos = boletos.order_by('-fecha_subida')
...
def reintentar_parseo(request, boleto_id):
    ...
    if not request.user.is_superuser and hasattr(request.user, 'agencias'):
        ua = request.user.agencias.filter(activo=True).first()
        if not ua or (boleto.agencia and boleto.agencia != ua.agencia):
             return Response({'error': 'No tiene permiso para acceder a este boleto'}, status=status.HTTP_403_FORBIDDEN)
...
def solicitar_anulacion(request):
    ...
    if not request.user.is_superuser and hasattr(request.user, 'agencias'):
        ua = request.user.agencias.filter(activo=True).first()
        if not ua or (boleto.agencia and boleto.agencia != ua.agencia):
             return Response({'error': 'No tiene permiso para este boleto'}, status=status.HTTP_403_FORBIDDEN)
...
def detalle_boleto(request, boleto_id):
    ...
    if not request.user.is_superuser and hasattr(request.user, 'agencias'):
        ua = request.user.agencias.filter(activo=True).first()
        if not ua or (boleto.agencia and boleto.agencia != ua.agencia):
                return Response({'error': 'No tiene permiso'}, status=status.HTTP_403_FORBIDDEN)

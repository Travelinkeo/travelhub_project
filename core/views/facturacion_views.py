...
class FacturacionDashboardView(SaaSMixin, LoginRequiredMixin, ListView):
    model = Factura
    ...
    def get_queryset(self):
        ...
        queryset = super().get_queryset().select_related('cliente', 'moneda').order_by('-fecha_emision')
...
class FacturaDetailView(SaaSMixin, LoginRequiredMixin, DetailView):
    model = Factura
...
@agency_role_required(['admin', 'gerente', 'contador']) # Contador también puede facturar según rol clásico
...
@agency_role_required(['admin', 'gerente']) # Ojo: Solo roles gerenciales pueden emitir definidamente

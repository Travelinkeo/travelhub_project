...
class GodModeDashboardView(UserPassesTestMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        ultimos_boletos = BoletoImportado.all_objects.select_related("agencia").order_by(
            "-fecha_subida"
        )[:5]
        ...
        impersonation_logs = AuditLog.objects.filter(accion="IMPERSONATE").order_by("-creado")[:20]
...
class ImpersonateAgencyView(UserPassesTestMixin, View):
    ...
    def get(self, request, agencia_id, *args, **kwargs):
        ...
        try:
            ...
            cache.set(rate_key, 1, timeout=GOD_MODE_RATE_WINDOW)
            count = 1

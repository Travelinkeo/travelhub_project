...
class GDSAnalysisAjaxView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        try:
            ...
            return render(
                request, "core/intelligence/partials/analysis_results.html", {"data": analysis_data}
            )
        except json.JSONDecodeError:
            logger.warning("GDS Analysis: body no es JSON válido")
            return JsonResponse({"status": "error", "message": "JSON inválido"}, status=400)
        except (ValidationError, KeyError, TypeError) as e:
            logger.warning("GDS Analysis: datos inválidos - %s", e)
            return JsonResponse({"status": "error", "message": f"Datos inválidos: {e}"}, status=400)
        except Exception:
            logger.exception("Error procesando GDS Analysis AJAX")
            return JsonResponse(
                {
                    "status": "error",
                    "message": "Error interno al analizar GDS. Revise el log de soporte.",
                },
                status=500,
            )
...
class GDSInjectERPView(LoginRequiredMixin, View):
        ...
        try:
            ...
            if not boletos_list:
                ...
                if "itinerario" in root_data:
                    ...
                    return JsonResponse(
                        {"status": "error", "message": "No se encontraron boletos en el análisis."},
                        status=400,
                    )
            ...
            if not agencia:
                return JsonResponse(
                    {"status": "error", "message": "Agencia no encontrada."}, status=400
                )
            ...
            with transaction.atomic():
                ...
                if pagador_id:
                    cliente_pagador = Cliente.objects.filter(
                        id_cliente=pagador_id, agencia=agencia
                    ).first()
                ...
                venta = VentaBuilderService.construir_venta_multipax(
                    agencia, boletos_list, cliente_pagador
                )
            ...
            logger.warning("GDS Inject: body no es JSON válido")
            return JsonResponse({"status": "error", "message": "JSON inválido"}, status=400)
        except (ValidationError, KeyError, TypeError, ValueError) as e:
            logger.warning("GDS Inject: datos inválidos - %s", e)
            return JsonResponse({"status": "error", "message": f"Datos inválidos: {e}"}, status=400)
        except (IntegrityError, DatabaseError):
            logger.exception("Error de BD inyectando GDS al ERP")
            return JsonResponse(
                {"status": "error", "message": "Error de base de datos al crear la venta"},
                status=500,
            )
        except Exception:
            logger.exception("Error inyectando GDS al ERP")
            return JsonResponse(
                {
                    "status": "error",
                    "message": "Error interno al crear la venta. Revise el log de soporte.",
                },
                status=500,
            )
    ...
    def _resolver_cliente(self, b_data, agencia):
        ...
        if doc and str(doc).strip() not in ("None", "", "N/A"):
            cliente = Cliente.objects.filter(
                cedula_identidad=str(doc)[:20], agencia=agencia
            ).first()

        if not cliente:
            cliente = Cliente.objects.filter(
                nombres__iexact=nombres, apellidos__iexact=apellidos, agencia=agencia
            ).first()
    ...
    def _sanitizar_boleto_data(self, b):
        ...
        if "nombre_aerolinea" in b:
            b["nombre_aerolinea"] = str(b["nombre_aerolinea"])[:3].upper()
        if "moneda" in b:
            b["moneda"] = str(b["moneda"])[:3].upper()

        if "itinerario" in b and isinstance(b["itinerario"], list):
            for seg in b["itinerario"]:
                if "aerolinea" in seg:
                    seg["aerolinea"] = str(seg["aerolinea"])[:3].upper()
                if "origen" in seg:

...
@extend_schema_view(
    generar=extend_schema(
        description="Genera el libro de ventas para un período específico en formato JSON o CSV.",
        responses={200: {"description": "Libro de ventas generado exitosamente"}},
    ),
    resumen_mensual=extend_schema(
        description="Obtiene el resumen mensual del libro de ventas con totales.",
        responses={200: {"description": "Resumen mensual del libro de ventas"}},
    ),
)
class LibroVentasViewSet(viewsets.ViewSet):
    ...
    def resumen_mensual(self, request):
        ...
        if not mes or not anio:
            return Response({"error": "Se requieren mes (1-12) y anio (YYYY)"}, status=400)

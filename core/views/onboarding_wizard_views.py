...
STEPS_DATA = {
    "welcome": {
        "title": "Bienvenido a TravelHub",
        "subtitle": "Te guiaremos en los primeros pasos",
    },
    "profile": {
        "title": "Perfil de la Agencia",
        "subtitle": "Completa la información de tu agencia",
    },
    "team": {"title": "Invita a tu Equipo", "subtitle": "Agrega usuarios y asigna roles"},
    "fiscal": {
        "title": "Configuración Fiscal",
        "subtitle": "Define tu régimen y preferencias fiscales",
    },
...
class OnboardingWizardView(LoginRequiredMixin, View):
    ...
    def _advance(self, progress, step, request, agencia):
        ...
        if next_step == "done" or step == "done":
            return render(
                request,
                "core/onboarding/steps/done.html",
                {
                    "current_agency": agencia,
                },
            )
    ...
    def _render_step(self, request, agencia, progress, step):
        ...
        if step == "profile":
            ...
            ctx["usuarios"] = UsuarioAgencia.objects.filter(agencia=agencia).select_related(
                "usuario"
            )
        elif step == "fiscal":

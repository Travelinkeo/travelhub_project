...
@extend_schema(
    description="Subir y procesar una imagen de pasaporte con OCR para extraer datos automáticamente.",
    request={
        "multipart/form-data": {
            "schema": {
                "type": "object",
                "properties": {
                    "passport_image": {
                        "type": "string",
                        "format": "binary",
                        "description": "Imagen del pasaporte (JPG o PNG)",
                    }
                },
                "required": ["passport_image"],
            }
        }
    },
    responses={
        201: {"description": "Pasaporte procesado exitosamente con datos extraídos"},
        400: {"description": "Error de validación: imagen no proporcionada o formato inválido"},
    },
    tags=["Pasaportes"],
)
@api_view(["POST"])
...
def upload_passport(request):
    ...
    if "passport_image" not in request.FILES:
        return Response(
            {"error": "No se proporcionó imagen de pasaporte"}, status=status.HTTP_400_BAD_REQUEST
        )
        ...
        return Response(
            {
                ...
                "data": {
                    ...
                    "fecha_nacimiento": str(pasaporte.fecha_nacimiento)
                    if pasaporte.fecha_nacimiento
                    else "",
                    "fecha_vencimiento": str(pasaporte.fecha_vencimiento)
                    if pasaporte.fecha_vencimiento
                    else "",
...
@extend_schema(
    description="Crear o actualizar un cliente en el CRM desde los datos extraídos de un pasaporte escaneado.",
    responses={
        200: {"description": "Cliente actualizado con datos del pasaporte"},
        201: {"description": "Cliente creado exitosamente desde datos del pasaporte"},
        404: {"description": "Pasaporte no encontrado"},
    },
    tags=["Pasaportes"],
)
@api_view(["POST"])
...
def create_client_from_passport(request, passport_id):
    ...
    try:
        ...
        return Response({"error": "Pasaporte no encontrado"}, status=status.HTTP_404_NOT_FOUND)
    except Exception as e:
        return Response(

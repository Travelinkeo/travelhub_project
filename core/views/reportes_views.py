...
@extend_schema(
    description="Obtener el libro diario contable con filtros por fecha.",
    parameters=[
        {
            "name": "fecha_desde",
            "in": "query",
    ...
    responses={200: {"description": "Libro diario con todos los asientos contables del período"}},
    tags=["Contabilidad"],
)
@api_view(["GET"])
@internal_auth
...
def libro_diario(request):
    ...
    asientos_qs = AsientoContable.objects.all().prefetch_related(
    ...
    return render(
        request, "core/reportes/libro_diario.html", {"asientos": libro, "active_tab": "reportes"}
    )


@extend_schema(
    description="Obtener el balance de comprobación (sumas y saldos de todas las cuentas).",
    parameters=[
        {
            "name": "fecha_hasta",
            "in": "query",
    ...
    responses={
        200: {
            "description": "Balance de comprobación con sumas, saldos y naturaleza de cada cuenta"
        }
    },
    tags=["Contabilidad"],
)
...
@extend_schema(
    description="Validar que todos los asientos contables estén cuadrados (debe = haber).",
    responses={
        200: {
            "description": "Resultado de la validación: lista de asientos descuadrados si existen"
        }
    },
    tags=["Contabilidad"],
)
...
def validar_cuadre(request):
    ...
    return render(
        request, "core/reportes/validacion.html", {"validacion": context, "active_tab": "reportes"}
    )


@extend_schema(
    description="Exportar el libro diario a formato Excel (.xlsx) para el período especificado.",
    parameters=[
        {
            "name": "fecha_desde",
            "in": "query",
    ...
    responses={200: {"description": "Archivo Excel (.xlsx) del libro diario"}},
    tags=["Contabilidad"],
)
@api_view(["GET"])
@internal_auth
...
def exportar_excel(request):
    ...
    try:
        ...
        return Response(
            {"error": "openpyxl no instalado. Ejecute: pip install openpyxl"}, status=500
        )
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="libro_diario_{datetime.now().strftime("%Y%m%d")}.xlsx"'
    )
    wb.save(response)
...
@extend_schema(
    description="Obtener el Estado de Resultados (P&L) con ingresos, costos, gastos y márgenes basado en movimientos contables (Clases 4, 5, 6).",
    parameters=[
        {
            "name": "fecha_desde",
            "in": "query",
    ...
    responses={
        200: {
            "description": "Estado de Resultados con ingresos, costos, gastos, utilidad bruta y neta"
        }
    },
    tags=["Contabilidad"],
)

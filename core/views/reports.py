...
class ReportesVentasView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Data Services
        logger.debug(
            f"ReportesVentasView - Selected Year: {selected_year} (Type: {type(selected_year)})"
        )

        direct_count = Venta.objects.filter(fecha_venta__year=selected_year).count()
        logger.debug(f"Direct Query Count in View for {selected_year}: {direct_count}")
        ...
        logger.debug(f"KPIs: {context['kpis']}")
        logger.debug(f"Sales Chart: {context['sales_chart']}")

        # Selectores
        # Obtener lista de años disponibles
        available_years = (
            Venta.objects.annotate(year=ExtractYear("fecha_venta"))
            .values_list("year", flat=True)
            .order_by("-year")
            .distinct()
        )
        context["available_years"] = (
            sorted(set(available_years), reverse=True) if available_years else [current_year]
        )

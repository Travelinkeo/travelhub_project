...
def check_recent_tickets():
    ...
    recent_tickets = BoletoImportado.objects.filter(fecha_subida__gte=time_threshold).order_by(
        "-fecha_subida"
    )
    ...
    for t in recent_tickets:
        print(
            f"[{t.fecha_subida.strftime('%Y-%m-%d %H:%M')}] ID: {t.pk} - Estado: {t.estado_parseo}"
        )

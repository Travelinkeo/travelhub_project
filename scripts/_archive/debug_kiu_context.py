...
def test_kiu_context():
    ...
    if not isinstance(agencia_data, dict):
        agencia_data = {}

    agente = (
        agencia_data.get("nombre")
        or agencia_data.get("iata")
        or data.get("AGENTE_EMISOR")
        or data.get("agente_emisor", "")
    )
    direccion_agencia = (
        agencia_data.get("direccion")
        or data.get("DIRECCION_AEROLINEA")
        or data.get("direccion_aerolinea", "")
    )

    context = {
        "boleto": {
            "pasajero_nombre_completo": data.get("passenger_name")
            or data.get("NOMBRE_DEL_PASAJERO", ""),
            "solo_nombre_pasajero": data.get("SOLO_NOMBRE_PASAJERO")
            or data.get("solo_nombre_pasajero", ""),
            "codigo_identificacion": data.get("passenger_document")
    ...
    missing = []
    if not boleto["fecha_emision"]:
        missing.append("fecha_emision")
    if not boleto["agente_emisor"]:
        missing.append("agente_emisor")
    if not boleto["direccion_aerolinea"]:
        missing.append("direccion_aerolinea")

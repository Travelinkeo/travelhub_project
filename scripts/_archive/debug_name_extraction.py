...
def _get_nombre_completo_pasajero(texto: str) -> str:
    ...
    match_hola = re.search(
        r"(?:Hola|Estimado|Hello|Dear)\s*[,:]?\s*([A-Z/ ]{3,})(?:\.|,)", texto, re.IGNORECASE
    )
    if match_hola:
    ...
    val = _extract_field_single_line(
        texto,
        [r"NAME/NOMBRE\s*[:\s]*(.+)", r"NAME\s*[:\s]*(.+)", r"NOMBRE DEL PASAJERO\s*[:\s]*(.+)"],
    )

...
def test_manual_notify():
    ...
    print(
        f"   PDF Generado: {boleto.archivo_pdf_generado.name if boleto.archivo_pdf_generado else 'NINGUNO'}"
    )
    ...
    if not boleto.archivo_pdf_generado:
        print(
            "⚠️ El boleto no tiene PDF generado. Intentando usar el archivo original como prueba..."
        )
        pdf_file_input = (
            boleto.archivo_boleto.path if hasattr(boleto.archivo_boleto, "path") else None
        )
    else:
        try:
    ...
    if store_id and store_id != "NO_DEFINIDO":
        ...
        try:
            res = TelegramNotificationService.send_document(
                file_path=pdf_file_input, caption=caption, chat_id=store_id
            )
    ...
    if group_id and group_id != "NO_DEFINIDO":
        ...
        try:
            res = TelegramNotificationService.send_document(
                file_path=pdf_file_input, caption=caption, chat_id=group_id
            )

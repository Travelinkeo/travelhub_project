...
def parse_kiu_flight_segments(itinerary_text):
    ...
    patterns = [
        re.compile(
            r"(\d{1,2}[A-Z]{3}\d{0,2})\s+([A-Z0-9]{2,3}\s*\d{1,4}[A-Z]?)\s+([A-Z]{3})\s*(\d{4})\s*-?\s*([A-Z]{3})\s*(\d{4})(.*)"
        ),
        re.compile(
            r"(\d{1,2}[A-Z]{3}\d{0,2})\s+([A-Z0-9]{2,3}\s*\d{1,4}[A-Z]?)\s+([A-Z]{3})\s+(\d{4})\s+([A-Z]{3})\s+(\d{4})(.*)"
        ),
    ]
    conviasa_pattern = re.compile(
        r"^([A-Z \.]+)\s+([A-Z0-9]{2}\s*\d{3,4})\s+([A-Z])\s+(\d{1,2}[A-Z]{3})\s+(\d{4})\s+(\d{4})(.*)"
    )
...
if __name__ == "__main__":
    ...
    messy_text = (
        "<B>CARACAS </B>5R1327 T <B>17FEB 0800</B> <B>0840</B> TLOW 23K OK\n<B> BARQUISIMETO </B>"
    )

...
from core.models import Ciudad, Moneda, ProductoServicio, Proveedor
...
def seed_data():
    ...
    cliente, _ = Cliente.objects.get_or_create(
        nombres="Cliente Demo", apellidos="Test", defaults={"cedula_identidad": "V-12345678"}
    )
    proveedor, _ = Proveedor.objects.get_or_create(nombre="Proveedor Demo")
    producto, _ = ProductoServicio.objects.get_or_create(
        nombre="Boleto Aéreo", codigo_interno="TKT001", defaults={"tipo_producto": "AIR"}
    )
    ...
    for month in range(1, 13):
        if month > timezone.now().month + 1:
            break  # Don't go too far into future
        ...
        for _ in range(random.randint(1, 3)):
            ...
            venta = Venta.objects.create(
                ...
                estado=random.choice(
                    [Venta.EstadoVenta.PAGADA_TOTAL, Venta.EstadoVenta.CONFIRMADA]
                ),

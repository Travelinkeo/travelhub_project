...
def test_automation():
    ...
    user, _ = User.objects.get_or_create(
        username="test_automation_user", defaults={"email": "test_auto@test.com"}
    )
    ...
    boleto = BoletoImportado.objects.create(
        datos_parseados=datos_parseados, agencia=agencia, formato_detectado="PDF"
    )
    ...
    if checks.exists():
        check = checks.first()
        print("✅ MigrationCheck creado automáticamente!")

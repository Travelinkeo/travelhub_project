...
def test_with_passenger():
    ...
    check = service.check_migration_requirements(pasajero_id=pasajero.id_pasajero, vuelos=vuelos)
    ...
    print(
        f"   Vacunas: {', '.join(check.vaccination_required) if check.vaccination_required else 'Ninguna'}"
    )

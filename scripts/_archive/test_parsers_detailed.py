...
def test_parser_with_sample(parser, parser_name, file_path):
    ...
    try:
        ...
        if result_dict.get("flights"):
            ...
            print(
                f"    Route: {f.get('origen', {}).get('ciudad', 'N/A')} → {f.get('destino', {}).get('ciudad', 'N/A')}"
            )
...
def main():
    ...
    for parser, stats in by_parser.items():
        ...
        print(
            f"  Detection: {stats['can_parse']}/{stats['total']} ({stats['can_parse'] / stats['total'] * 100:.0f}%)"
        )
        print(
            f"  Extraction: {stats['success']}/{stats['total']} ({stats['success'] / stats['total'] * 100:.0f}%)"
        )

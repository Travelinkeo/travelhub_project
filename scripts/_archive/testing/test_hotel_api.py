...
def obtener_catalogos(token):
    ...
    if response.status_code == 200:
        ...
        catalogos["proveedores"] = (
            proveedores["results"] if "results" in proveedores else proveedores
        )
...
def crear_venta_hotel(token, catalogos):
    ...
    if not producto_hotel:
        ...
        producto_hotel = (
            catalogos["productos"][0] if catalogos["productos"] else {"id_producto_servicio": 1}
        )
...
def main():
    ...
    if venta:
        ...
        print(
            f"Puedes ver la venta en: http://localhost:8000/admin/core/venta/{venta['id_venta']}/change/"
        )
    else:

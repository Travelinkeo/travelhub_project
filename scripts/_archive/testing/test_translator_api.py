...
def get_jwt_token():
    ...
    try:
        ...
        user = User.objects.create_user(
            username="test_translator", password="test123456", email="test@example.com"
        )
        print("✓ Usuario de prueba creado")

    # Obtener token JWT
    response = requests.post(
        "http://127.0.0.1:8000/api/auth/jwt/obtain/",
        {"username": "test_translator", "password": "test123456"},
    )
    ...
    for test in tests:
        ...
        try:
            response = test_api_endpoint(
                test["url"], test.get("method", "GET"), test.get("data"), token
            )
            ...
            if response.status_code == 200:
                ...
                if data.get("success", False):
                    ...
                    if "supported_gds" in data:
                        ...
                        print(
                            f"   📝 Validación: {validation['valid_lines']}/{validation['total_lines']} líneas válidas"
                        )
                    elif "translated_itinerary" in data:
                        print("   🔄 Itinerario traducido exitosamente")
                    ...
                    print("   ❌ FALLO - Respuesta sin success=True")

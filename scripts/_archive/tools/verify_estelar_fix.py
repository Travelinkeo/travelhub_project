...
def test_estelar_fix():
    ...
    try:
        ...
        if (
            t["NUMERO_DE_BOLETO"] == expected_tkt
            and t["NOMBRE_DEL_PASAJERO"] == expected_pax
            and t["CODIGO_RESERVA"] == expected_pnr
        ):
            print("\nSUCCESS: All key fields extracted correctly!")
        else:
            print("\nPARTIAL SUCCESS/FAILURE: Some fields mismatch.")
            if t["NUMERO_DE_BOLETO"] != expected_tkt:
                print(f"Ticket mismatch: Got {t['NUMERO_DE_BOLETO']}, Expected {expected_tkt}")
            if t["NOMBRE_DEL_PASAJERO"] != expected_pax:
                print(f"Pax mismatch: Got {t['NOMBRE_DEL_PASAJERO']}, Expected {expected_pax}")
            if t["CODIGO_RESERVA"] != expected_pnr:
                print(f"PNR mismatch: Got {t['CODIGO_RESERVA']}, Expected {expected_pnr}")

...
def verify_alert():
    ...
    user, _ = User.objects.get_or_create(
        username="test_alert_user", defaults={"email": "alert@test.com"}
    )

    # Agencia requiere propietario (non-nullable)
    agencia, _ = Agencia.objects.get_or_create(
        nombre="Agencia Alerta Test",
        defaults={"propietario": user, "email_principal": "test@travelhub.com"},
    )
    ...
    print(
        f"✅ Usuario {user.username} configurado con Telegram ID: {usuario_agencia.telegram_chat_id}"
    )

    # 2. Crear Venta
    moneda, _ = Moneda.objects.get_or_create(codigo_iso="USD", defaults={"nombre": "Dolar"})
    venta, _ = Venta.objects.get_or_create(
        localizador="ALERTA001", defaults={"agencia": agencia, "creado_por": user, "moneda": moneda}
    )
    print(f"✅ Venta obtenida/creada: {venta.localizador}")

    # 3. Crear Pasajero
    pasajero, _ = Pasajero.objects.get_or_create(
        numero_documento="V00000000", defaults={"nombres": "Pax", "apellidos": "Alerta"}
    )
    ...
    print(
        "👀 Revisa los logs de la consola o 'travelhub.log' para ver 'Alerta migratoria enviada a...'"
    )
    print(
        "(Nota: Como el ID 123456789 es falso, Telegram probablemente retornará error de chat not found, pero el intento cuenta)"
    )

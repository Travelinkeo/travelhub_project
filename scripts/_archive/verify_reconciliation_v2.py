...
def verify_reconciliation_v2():
    ...
    agencia, _ = Agencia.objects.get_or_create(
        nombre="Agencia AI Test", defaults={"activa": True, "propietario": propietario}
    )
...
"""
    ...
    reporte = ReporteProveedor.objects.create(
        proveedor=proveedor, agencia=agencia, estado=ReporteProveedor.EstadoReporte.PENDIENTE
    )
    reporte.archivo.save("bsp_messy_report.csv", ContentFile(csv_content))
    print("(PROCESS) Reporte cargado. Ejecutando motor de IA...")
    ...
    try:
        ...
        for item in reporte.items.all().order_by("numero_boleto"):
            ...
            print(
                f"   [{status}] Tkt: {item.numero_boleto} | Pasajero: {item.pasajero} | Sistema: ${item.monto_sistema} | Proveedor: ${item.monto_total_proveedor}"
            )

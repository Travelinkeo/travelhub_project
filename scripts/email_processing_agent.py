    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            try:
                ...
                logging.error(
                    f"Error: No se encontró '{CREDENTIALS_FILE}'. Descárgalo desde tu Google Cloud Console y colócalo en la carpeta /scripts."
                )
                return None
        ...
        def extract_parts(parts_list):
            ...
            for part in parts_list:
                ...
                if mime_type == "text/plain":
                    ...
                    if body_data := part.get("body", {}).get("data"):
                        soup = BeautifulSoup(
                            base64.urlsafe_b64decode(body_data).decode("utf-8", "ignore"), "lxml"
                        )
                    ...
                    if filename := part.get("filename", ""):
                        if filename.lower().endswith(".pdf"):
                            ...
                            if pdf_data := att.get("data"):
                                pdf_text = ""
                                with fitz.open(
                                    stream=base64.urlsafe_b64decode(pdf_data), filetype="pdf"
                                ) as doc:
                                    for page in doc:
...
def main():
    ...
    for message in unread_messages:
        ...
        try:
            ...
            logging.info(
                f"Correo ID: {msg_id} procesado y guardado en la BD con ID: {comunicacion.id}"
            )
            ...
            if category == ComunicacionProveedor.Categoria.INFO:
                logging.info(
                    "Detectada comunicación de tipo [Información General]. Guardando en Base de Conocimiento..."
                )
                crear_articulo_desde_gemini(
                    datos_gemini=extracted_json,
                    categoria=extracted_json.get(
                        "categoria_sugerida", "Información de Proveedores"
                    ),
            ...
            # Si la comunicación es urgente, activar el manejador de notificaciones
                ...
                logging.info(
                    "Detectada notificación urgente. Se requiere acción manual o un manejador específico."
                )
            ...
            # Si es una promoción, generar contenido de marketing
                ...
                logging.info(
                    "Detectada comunicación de tipo [Promoción]. Generando contenido de marketing..."
                )
                creative_content = generate_promotional_content(extracted_json)
                if "error" in creative_content:
                    logging.error(
                        f"Error al generar contenido de marketing: {creative_content.get('error')}"
                    )
                else:
                    ...
                    comunicacion.save()
                    logging.info(
                        f"Contenido de marketing generado y guardado para la comunicación ID: {comunicacion.id}"
                    )

...
datos_boleto = {
    "reservation_code": "MSYHMI",
    "passenger_name": "VANESSA ALEJANDRA ESPINALES TORREALBA",
    "document_number": "180580453",
    "contact_email": "ventas1mydestiny@gmail.com",
    "total_paid": "514.47",
    "currency": "USD",
...
def get_city_by_name(city_string):
    ...
    try:
        ...
        raise Exception(
            f"No se pudo encontrar una ciudad con el nombre '{city_name}' en la base de datos."
        )
    except Ciudad.MultipleObjectsReturned:
        # Si hay múltiples, intentar filtrar por país si es posible (ej. Colombia para BOG)
        if "BOG" in city_string:
            return Ciudad.objects.get(nombre__iexact=city_name, pais__codigo_iso_2="CO")
        raise Exception(
            f"Se encontraron múltiples ciudades con el nombre '{city_name}'. Se necesita un criterio más específico."
        )


def run():
    print("Iniciando el proceso final de creación/actualización de Venta y generación de PDF...")
        ...
        with transaction.atomic():
            ...
            pasajero, _ = Pasajero.objects.get_or_create(
                ...
                defaults={
                    ...
                    "email": datos_boleto["contact_email"],
                },
            )
            ...
            for i, flight_data in enumerate(datos_boleto["flights"]):
                fecha_salida = parse_flight_datetime(
                    flight_data["date"], flight_data["departure_time"]
                )

                ciudad_origen = get_city_by_name(flight_data["departure_airport"])
                ciudad_destino = get_city_by_name(flight_data["arrival_airport"])

                # CORRECCIÓN FINAL: Usar 'aerolinea' en lugar de 'operado_por'
                SegmentoVuelo.objects.get_or_create(
                    ...
                    defaults={
                        "fecha_llegada": parse_flight_datetime(
                            flight_data["date"], flight_data["arrival_time"]
                        ),
                        "aerolinea": flight_data[
                            "operated_by"
                        ],  # Usar el valor de 'operado por' aquí
                    },
                )

...
def test_kiu_multiline():
    ...
    # Assertions
    assert data.issue_date == "05 FEB 2026" or data.issue_date == "05FEB26", (
        f"Failed Date: {data.issue_date}"
    )
    assert data.agency.get("nombre") == "BLA12345", f"Failed Agent: {data.agency.get('nombre')}"
    assert data.agency.get("direccion") == "PRINCIPAL AV VENEZUELA", (
        f"Failed Address: {data.agency.get('direccion')}"
    )

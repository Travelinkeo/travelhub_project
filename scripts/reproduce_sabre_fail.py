    ...
    try:
        ...
        for fpath in files:
            ...
            try:
                ...
                with pdfplumber.open(fpath) as pdf:
                    ...
                    for i, page in enumerate(pdf.pages):
                        ...
                        log_print(
                            f"  Page {i + 1} extracted bytes: {len(extracted) if extracted else 0}"
                        )
                        text += (extracted or "") + "\n"
            ...
            vuelo_section_match = re.search(
                r"(?:Información [Dd]e Vuelo|Flight Information)(.*?)(?:Detalles [Dd]e Pago|Payment Details|Aviso:|Notice:)",
                text,
            ...
            if vuelo_section_match:
                ...
                # DEBUG SPLIT
                primary_blocks = re.split(
                    r"(?:This is not a boarding pass|Esta no es una tarjeta de embarque|Preparado para)",
                    vuelo_text,
                    flags=re.IGNORECASE,
                )
            ...
            try:
                ...
                for i, f in enumerate(flights):
                    ...
                    log_print(
                        f"      - Raw Block Snippet: {f.get('aerolinea')} {f.get('numero_vuelo')}"
                    )

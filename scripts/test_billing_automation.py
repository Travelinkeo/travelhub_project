...
def test_billing_logic():
    ...
    try:
        user, _ = User.objects.get_or_create(
            username="billing_tester", defaults={"email": "tester@billing.com"}
        )

        agencia, created = Agencia.objects.get_or_create(
            nombre="Test Agency Billing",
            defaults={"email_principal": "billing@test.com", "plan": "FREE", "propietario": user},
        )
        if not created:
        ...
        print(
            f"   Agencia creada: {agencia.nombre} (Plan: {agencia.plan}, Ventas Límite: {agencia.limite_ventas_mes})"
        )
    ...
    with (
        ...
        mock_session_create.return_value = MagicMock(
            url="https://checkout.stripe.com/test", id="cs_test_abc"
        )
    ...
    mock_event = {
        ...
        "data": {
            "object": {
                ...
                "metadata": {"agencia_id": agencia.id, "plan": "PRO"},
            }
        },
    }

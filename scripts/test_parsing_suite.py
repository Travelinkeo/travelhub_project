...
def run_debug():
    ...
    for filename in target_files:
        ...
        is_kiu = (
            ...
            or "PASSENGER ITINERARY RECEIPT" in plain_text_upper
            or ("ISSUE AGENT" in plain_text_upper and "FROM/TO" in plain_text_upper)
            or "AVIOR AIRLINES" in plain_text_upper
        )
        ...
        try:
            ...
            for i, v in enumerate(vuelos):
                print(
                    f"  [{i}] {v.get('fecha')} {v.get('aerolinea')} {v.get('numero_vuelo')} {v.get('origen')}-{v.get('destino')}"
                )

...
def crear_reemision(boleto_original):
    ...
    print("   ✅ Versionado aplicado")
    print(f"   📊 Versión final: {boleto_v2.version}")
    print(f"   📌 Estado emisión final: {boleto_v2.estado_emision}")
    print(
        f"   🔗 Boleto padre: {boleto_v2.boleto_padre.id_boleto_importado if boleto_v2.boleto_padre else 'None'}"
    )
    print()
...
def verificar_versionado(boleto_v1, boleto_v2):
    ...
    if boleto_v2.boleto_padre != boleto_v1:
        errores.append("❌ Boleto v2 no apunta al v1 como padre")
    else:
        print("   ✅ Boleto v2 apunta correctamente al v1 como padre")

    # Verificar que v1 no tenga padre
    if boleto_v1.boleto_padre is not None:
        errores.append("❌ Boleto v1 tiene padre (debería ser None)")
    else:
        print("   ✅ Boleto v1 no tiene padre (correcto)")
...
def mostrar_historial():
    ...
    boletos = BoletoImportado.objects.filter(numero_boleto="1234567890").order_by("version")
    ...
    for boleto in boletos:
        ...
        print(
            f"      Padre: {boleto.boleto_padre.id_boleto_importado if boleto.boleto_padre else 'N/A'}"
        )

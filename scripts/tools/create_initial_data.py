...
from core.models import Agencia, Moneda, UsuarioAgencia
...
try:
    ...
    if not admin_user:
        print(
            "❌ Error: No se encontró el usuario 'admin'. Por favor, crea un superusuario primero."
        )
        exit(1)

    agencia, created = Agencia.objects.get_or_create(
        nombre="Agencia Central", defaults={"iata": "123456", "propietario": admin_user}
    )
    if created:
    ...
    ua, ua_created = UsuarioAgencia.objects.get_or_create(
        usuario=admin_user, agencia=agencia, defaults={"rol": "admin"}
    )
    if ua_created:
        print(
            f"✅ Usuario '{admin_user.username}' asignado como ADMIN a la agencia '{agencia.nombre}'."
        )
    else:
        print(
            f"ℹ️ Usuario '{admin_user.username}' ya estaba asignado a la agencia '{agencia.nombre}'."
        )

    moneda, mon_created = Moneda.objects.get_or_create(
        codigo_iso="USD", defaults={"nombre": "Dolar Estadounidense", "simbolo": "$"}
    )
    if mon_created:

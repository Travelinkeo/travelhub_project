...
def fix_golden_ticket():
    ...
    content = content.replace(
        "| default('#0D1E40', true if agencia else '#0D1E40')", '| default:"#0D1E40"'
    )
    content = content.replace(
        "| default('#2173A6', true if agencia else '#2173A6')", '| default:"#2173A6"'
    )

    # 2. Fix the SOURCE_SYSTEM in root
    content = content.replace(
        "{{ agencia.color_secundario|default:'#88081F', true }}",
        "{{ agencia.color_secundario|default:'#88081F' }}",
    )
    content = content.replace(
        "{{ agencia.color_amadeus|default:'#0C66E1', true }}",
        "{{ agencia.color_amadeus|default:'#0C66E1' }}",
    )
    ...
    content = content.replace(
        "{{ 'logo-invertido' if is_dark_color else 'logo-oscuro' }}",
        "{% if is_dark_color %}logo-invertido{% else %}logo-oscuro{% endif %}",
    )
    content = content.replace(
        "{{ solo_nombre_pasajero|upper }}", "{{ solo_nombre_pasajero|upper }}"
    )
    content = content.replace(
        "{{ AGENTE_EMISOR|default:agencia.iata if agencia else '10617390' }}",
        "{% if agencia %}{{ AGENTE_EMISOR|default:agencia.iata }}{% else %}10617390{% endif %}",
    )

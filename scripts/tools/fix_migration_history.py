...
cur.execute(
    "SELECT app, name, applied FROM django_migrations WHERE app IN ('bookings', 'contabilidad') ORDER BY app, name;"
)
rows = cur.fetchall()
...
cur.execute(
    "SELECT COUNT(*) FROM django_migrations WHERE app='contabilidad' AND name='0002_initial';"
)
...
if count == 0:
    ...
    cur.execute(
        ...
        ("contabilidad", "0002_initial", datetime.now(UTC)),
    )
    conn.commit()

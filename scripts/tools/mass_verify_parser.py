    ...
    for folder in folders:
        ...
        for filename in files:
            ...
            try:
                ...
                report.append(
                    {"system": folder, "file": filename, "status": "🔥 ERROR", "error": str(e)}
                )

    # Print Summary Table
    print("\n" + "=" * 80)
    print(
        f"{'SISTEMA':<12} | {'SITUACIÓN':<10} | {'GDS PNR':<10} | {'AIRLINE PNR':<12} | {'VUELOS':<6}"
    )
    print("-" * 80)
    for r in report:
        print(
            f"{r['system']:<12} | {r['status']:<10} | {str(r.get('pnr')):<10} | {str(r.get('airline_pnr')):<12} | {r.get('flights_count', 0)}"
        )
    print("=" * 80)

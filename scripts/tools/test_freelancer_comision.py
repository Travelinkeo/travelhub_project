...
from apps.crm.models_freelancer import ComisionFreelancer
...
def probar_comision_freelancer():
    ...
    try:
        ...
        print(
            f"❌ Error: El freelancer no está configurado. Ejecuta crear_freelancer.py primero. ({e})"
        )
        return
    ...
    estelar_path = (
        r"C:\Users\ARMANDO\travelhub_project\core\tests\fixtures\venezuela_web\estelar_sample.eml"
    )
    ...
    with open(estelar_path, "rb") as f:
        boleto = BoletoImportado.objects.create(
            archivo_boleto=File(f, name="estelar_test_freelancer.eml"), estado_parseo="PEN"
        )
    ...
    comision = ComisionFreelancer.objects.filter(
        venta__localizador=boleto.localizador_pnr, freelancer=perfil
    ).first()
    ...
    if comision:
        ...
        print(
            "❌ Error: No se generó registro de comisión. Revisa los logs de TicketParserService."
        )
    print("-" * 50)

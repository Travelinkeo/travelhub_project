...
def verify_linkeo():
    ...
    agencia, _ = Agencia.objects.get_or_create(
        nombre="Agencia Linkeo Test",
        defaults={"propietario": user, "email_principal": "test@linkeo.com"},
    )
    ...
    def mock_detect_intent(self, text):
        ...
        if "Juan" in text:
            ...
            return {
                "intent": "QUERY_SALES",
                "params": {"date_range_start": timezone.now().date().isoformat()},
            }
        elif "visa" in text:
            return {
                "intent": "CHECK_MIGRATION",
                "params": {"nationality": "VEN", "destination": "ESP"},
            }

...
def verify_reconciliation():
    ...
    proveedor, _ = Proveedor.objects.get_or_create(
        nombre="Aerolinea Test", defaults={"rif": "J-12345678-9", "tipo_proveedor": "AIR"}
    )
...
"""
    ...
    reporte = ReporteProveedor.objects.create(
        proveedor=proveedor, estado=ReporteProveedor.EstadoReporte.PENDIENTE
    )
    ...
    for item in items:
        ...
        print(
            f"   - Ticket: {item.numero_boleto} -> {status_icon} (Monto Proveedor: {item.monto_total_proveedor})"
        )

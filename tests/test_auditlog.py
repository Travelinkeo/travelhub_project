...
def test_auditlog_itemventa_y_venta_eliminacion():
    ...
    prod, _ = ProductoServicio.objects.get_or_create(
        nombre="Servicio Test", defaults={"tipo_producto": "GEN"}
    )
    venta = Venta.objects.create(
        cliente=cliente, moneda=moneda, subtotal=Decimal("10.00"), impuestos=Decimal("2.00")
    )
    item = ItemVenta.objects.create(
        ...
        impuestos_item_venta=Decimal("2.00"),
    )

...
pytestmark = pytest.mark.skip(
    reason="Usa reverse('core:audit-log-list') pero la URL real es name='api_audit_logs' sin namespace; ademas la vista requiere modelo+object_id+agencia (no lista general). Verificado 2026-08-04."
)


@pytest.mark.django_db
def test_auditlog_api_list(api_client_autenticado):
    """test_auditlog_api_list."""
    ...
    prod, _ = ProductoServicio.objects.get_or_create(
        nombre="Servicio Auditado", defaults={"tipo_producto": "GEN"}
    )

    venta = Venta.objects.create(
        cliente=cliente, moneda=moneda, subtotal=Decimal("5.00"), impuestos=Decimal("1.00")
    )
    item = ItemVenta.objects.create(
        ...
        impuestos_item_venta=Decimal("1.00"),
    )
    # Eliminar item -> log
    item.delete()
...
def test_auditlog_api_filters(api_client_autenticado):
    ...
    venta = Venta.objects.create(
        cliente=cliente, moneda=moneda, subtotal=Decimal("0.00"), impuestos=Decimal("0.00")
    )

    # Generar un log creando y luego eliminando un ItemVenta asociado para garantizar existencia de logs filtrables
    prod, _ = ProductoServicio.objects.get_or_create(
        nombre="Filtro Audit", defaults={"tipo_producto": "GEN"}
    )
    item = ItemVenta.objects.create(
        venta=venta,
        producto_servicio=prod,
        cantidad=1,
        precio_unitario_venta=Decimal("0.00"),

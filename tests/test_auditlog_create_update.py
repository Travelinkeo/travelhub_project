...
def test_auditlog_venta_create_and_update():
    """test_auditlog_venta_create_and_update."""
    ...
    venta = Venta.objects.create(
        ...
        descripcion_general="Desc A",
    )
    create_log = AuditLog.objects.filter(
        modelo="Venta", accion="CREATE", object_id=str(venta.pk)
    ).first()
    ...
    venta.save()
    update_log = (
        AuditLog.objects.filter(modelo="Venta", accion="UPDATE", object_id=str(venta.pk))
        .order_by("-creado")
        .first()
    )
...
def test_auditlog_itemventa_create_and_update():
    ...
    log_create = AuditLog.objects.filter(
        modelo="ItemVenta", accion="CREATE", object_id=str(item.pk)
    ).first()
    ...
    item.save()
    log_update = (
        AuditLog.objects.filter(modelo="ItemVenta", accion="UPDATE", object_id=str(item.pk))
        .order_by("-creado")
        .first()
    )

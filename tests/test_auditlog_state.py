...
pytestmark = pytest.mark.skip(
    reason="Requiere log de cambio de estado en Venta; no hay signal post_save de estado. Verificado 2026-08-04."
)
...
def test_auditlog_state_change_creates_log():
    ...
    venta = Venta.objects.create(
        cliente=cliente, moneda=moneda, subtotal=Decimal("100.00"), impuestos=Decimal("0.00")
    )
    ...
    venta.save()
    log = (
        AuditLog.objects.filter(modelo="Venta", accion="STATE", object_id=str(venta.pk))
        .order_by("-creado")
        .first()
    )

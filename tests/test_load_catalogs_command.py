...
def test_load_catalogs_dry_run(temp_fixtures_dir, capsys):
    ...
    call_command(
        "load_catalogs", "--only", "paises", "--dir", str(temp_fixtures_dir.parent), "--dry-run"
    )
...
def test_load_catalogs_upsert_updates_existing(temp_fixtures_dir, capsys):
    ...
    call_command(
        "load_catalogs", "--only", "paises", "--dir", str(temp_fixtures_dir.parent), "--upsert"
    )
...
def test_load_catalogs_city_without_country_fails(temp_fixtures_dir):
    ...
    with pytest.raises(CommandError) as excinfo:
        call_command("load_catalogs", "--only", "ciudades", "--dir", str(temp_fixtures_dir.parent))

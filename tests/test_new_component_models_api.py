...
def test_create_and_list_new_components(api_client_staff, venta_base):
    """test_create_and_list_new_components."""
    venta = venta_base
    client = api_client_staff

    # Crear alquiler auto
    resp_auto = client.post(
        "/core/api/alquileres-autos/",
        {
    ...
    # Crear evento
    resp_evento = client.post(
        "/core/api/eventos-servicios/",
        {
            "venta": venta.id_venta,
    ...
    # Crear circuito + dia
    resp_circuito = client.post(
        "/core/api/circuitos-turisticos/",
        {"venta": venta.id_venta, "nombre_circuito": "Europa Express"},
        format="json",
    )
    assert resp_circuito.status_code in (200, 201), resp_circuito.content
    circuito_id = resp_circuito.json()["id_circuito"]

    resp_dia = client.post(
        "/core/api/circuitos-dias/",
        {"circuito": circuito_id, "dia_numero": 1, "titulo": "Llegada"},
        format="json",
    )
    assert resp_dia.status_code in (200, 201), resp_dia.content

    # Crear paquete aéreo
    resp_paquete = client.post(
        "/core/api/paquetes-aereos/",
        {"venta": venta.id_venta, "nombre_paquete": "Aereo + Hotel"},
        format="json",
    )
    assert resp_paquete.status_code in (200, 201), resp_paquete.content

    # Crear servicio adicional
    resp_servicio = client.post(
        "/core/api/servicios-adicionales/",
        {"venta": venta.id_venta, "tipo_servicio": "SEG"},
        format="json",
    )

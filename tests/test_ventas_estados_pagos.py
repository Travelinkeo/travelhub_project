...
def venta_base(api_client, cliente, moneda_usd, producto):
    ...
    payload = {
        ...
        "items_venta": [
            {
                "producto_servicio": producto.id_producto_servicio,
                "cantidad": 1,
                "precio_unitario_venta": "100.00",
                "impuestos_item_venta": "0.00",
            }
        ],
    }
...
def test_estado_pasa_a_parcial_con_pago(api_client, venta_base, moneda_usd):
    """test_estado_pasa_a_parcial_con_pago."""
    # Pago parcial 40
...
def test_estado_pasa_a_pagada_total_y_puntos(api_client, venta_base, moneda_usd, cliente):
    """test_estado_pasa_a_pagada_total_y_puntos."""
    # Pago total 100
    resp = api_client.post(
        "/api/pagos-venta/",
        {
...
def test_no_duplica_puntos_con_pagos_incrementales(api_client, venta_base, moneda_usd, cliente):
    """test_no_duplica_puntos_con_pagos_incrementales."""
    # Pago parcial 60
    r1 = api_client.post(
        "/api/pagos-venta/",
        {
...
def test_completada_manual_antes_de_pagos_otorga_puntos_una_vez(
    ...
    """test_completada_manual_antes_de_pagos_otorga_puntos_una_vez."""
...
def test_confirmada_luego_pagos_total_otorga_puntos(api_client, venta_base, moneda_usd, cliente):
    """test_confirmada_luego_pagos_total_otorga_puntos."""
    ...
    cliente.refresh_from_db()
    assert cliente.puntos_fidelidad == 0
    # Pagar total
    p = api_client.post(
        "/api/pagos-venta/",
        {
...
def test_venta_total_menor_a_10_no_otorga_puntos(api_client, cliente, moneda_usd, producto):
    """test_venta_total_menor_a_10_no_otorga_puntos."""
...
def test_idempotencia_puntos_recalculo_y_pagos_extra(api_client, venta_base, moneda_usd, cliente):
    """test_idempotencia_puntos_recalculo_y_pagos_extra."""
    ...
    assert p2.status_code == 201
    cliente.refresh_from_db()
    assert cliente.puntos_fidelidad == 10  # no aumenta
